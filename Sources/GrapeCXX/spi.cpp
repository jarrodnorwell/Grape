...
uint16_t Spi::crc16(uint32_t value, uint8_t *data, size_t size) {
    static const uint16_t table[] = {0xC0C1, 0xC181, 0xC301, 0xC601, 0xCC01, 0xD801, 0xF001,
                                     0xA001};
...
bool Spi::loadFirmware() {
    ...
    for (uint32_t addr = 0x1FE00; addr <= 0x1FF00; addr += 0x100) {
        ...
        firmware[addr + 0x06] = 'G'; // Nickname, char 1
        firmware[addr + 0x08] = 'r'; // Nickname, char 2
        firmware[addr + 0x0A] = 'a'; // Nickname, char 3
        firmware[addr + 0x0C] = 'p'; // Nickname, char 4
        firmware[addr + 0x0E] = 'e'; // Nickname, char 5
                    ...
                    case 6: // AUX input
                        if (writeCount & 1) {
                            ...
                            size_t index = std::min<size_t>(
                                    (core->getGlobalCycles() - micCycles) / micStep, micBufSize);
