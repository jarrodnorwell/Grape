...
bool Gpu::getFrame(uint32_t *out, bool gbaCrop) {
    ...
    if (gbaCrop) {
        ...
        if (Settings::getHighRes3D()) {
            ...
            for (int y = 0; y < 192; y++) {
                for (int x = 0; x < 256; x++) {
                    uint32_t color = rgb5ToRgb8((x >= 8 && x < 256 - 8 && y >= 16 && y < 192 - 16) ?
                                                ...
                                                core->memory.read<uint16_t>(0, base +
                                                                               (y * 256 + x) * 2));
            ...
            for (int y = 0; y < 192; y++) {
                for (int x = 0; x < 256; x++) {
                    out[offset + y * 256 + x] = rgb5ToRgb8(
                            (x >= 8 && x < 256 - 8 && y >= 16 && y < 192 - 16) ?
                            buffers.framebuffer[(y - 16) * 256 + (x - 8)] :
    ...
    switch (++vCount) {
        case 160: // End of visible scanlines
        {
            ...
            if (framebuffers.size() < 2) {
                ...
                memcpy(buffers.framebuffer, core->gpu2D[0].getFramebuffer(),
        ...
        if (displayCapture) {
            ...
            switch ((dispCapCnt & 0x60000000) >> 29) // Capture source
            {
                case 0: // Source A
                {
                    ...
                    uint32_t *source = (dispCapCnt & BIT(24)) ? core->gpu3DRenderer.getLine(vCount)
                ...
                case 1: // Source B
                {
                    ...
                    // Copy a scanline to memory
                    for (int i = 0; i < width; i++) {
                        uint16_t color = core->memory.read<uint16_t>(0, base +
                                                                        ((readOffset + i * 2) &
                                                                         0x1FFFF));
                        core->memory.write<uint16_t>(0, base + ((writeOffset + i * 2) & 0x1FFFF),
                                                     color);
                ...
                default: // Blended
                {
                    ...
                    for (int i = 0; i < width; i++) {
                        ...
                        uint16_t c2 = core->memory.read<uint16_t>(0, base + ((readOffset + i * 2) &
                                                                             0x1FFFF));

                        // Blend the color values
                        uint8_t r = std::min(
                                (((c1 >> 0) & 0x1F) * eva + ((c2 >> 0) & 0x1F) * evb) / 16, 31);
                        uint8_t g = std::min(
                                (((c1 >> 5) & 0x1F) * eva + ((c2 >> 5) & 0x1F) * evb) / 16, 31);
                        uint8_t b = std::min(
                                (((c1 >> 10) & 0x1F) * eva + ((c2 >> 10) & 0x1F) * evb) / 16, 31);
        ...
        case 192: // End of visible scanlines
        {
            ...
            if (framebuffers.size() < 2) {
                ...
                if (powCnt1 & BIT(0)) // LCDs enabled
                {
                    ...
                    {
                        memcpy(&buffers.framebuffer[0], core->gpu2D[0].getFramebuffer(),
                               256 * 192 * sizeof(uint32_t));
                        memcpy(&buffers.framebuffer[256 * 192], core->gpu2D[1].getFramebuffer(),
                               256 * 192 * sizeof(uint32_t));
                    } else {
                        memcpy(&buffers.framebuffer[0], core->gpu2D[1].getFramebuffer(),
                               256 * 192 * sizeof(uint32_t));
                ...
                if (Settings::getHighRes3D() && (core->gpu2D[0].readDispCnt() & BIT(3))) {
                    ...
                    memcpy(buffers.hiRes3D, core->gpu3DRenderer.getLine(0),
