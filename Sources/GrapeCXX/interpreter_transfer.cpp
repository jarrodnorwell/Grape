...
FORCE_INLINE int Interpreter::strbOf(uint32_t opcode, uint32_t op2) // STRB Rd,[Rn,op2]
{
    ...
    uint32_t op0 = *registers[(opcode & 0x0000F000) >> 12] +
                   (((opcode & 0x0000F000) == 0x0000F000) ? 4 : 0);
    uint32_t op1 = *registers[(opcode & 0x000F0000) >> 16];
...
int Interpreter::ldrsbOfrm(uint32_t opcode) {
    return ldrsbOf(opcode, -rp(opcode));
} // LDRSB Rd,[Rn,-Rm]
int Interpreter::ldrsbOfim(uint32_t opcode) {
    return ldrsbOf(opcode, -ipH(opcode));
} // LDRSB Rd,[Rn,-#i]
int Interpreter::ldrsbOfrp(uint32_t opcode) {
    return ldrsbOf(opcode, rp(opcode));
} // LDRSB Rd,[Rn,Rm]
int Interpreter::ldrsbOfip(uint32_t opcode) {
    return ldrsbOf(opcode, ipH(opcode));
} // LDRSB Rd,[Rn,#i]
int Interpreter::ldrsbPrrm(uint32_t opcode) {
    return ldrsbPr(opcode, -rp(opcode));
} // LDRSB Rd,[Rn,-Rm]!
int Interpreter::ldrsbPrim(uint32_t opcode) {
    return ldrsbPr(opcode, -ipH(opcode));
} // LDRSB Rd,[Rn,-#i]!
int Interpreter::ldrsbPrrp(uint32_t opcode) {
    return ldrsbPr(opcode, rp(opcode));
} // LDRSB Rd,[Rn,Rm]!
int Interpreter::ldrsbPrip(uint32_t opcode) {
    return ldrsbPr(opcode, ipH(opcode));
} // LDRSB Rd,[Rn,#i]!
int Interpreter::ldrsbPtrm(uint32_t opcode) {
    return ldrsbPt(opcode, -rp(opcode));
} // LDRSB Rd,[Rn],-Rm
int Interpreter::ldrsbPtim(uint32_t opcode) {
    return ldrsbPt(opcode, -ipH(opcode));
} // LDRSB Rd,[Rn],-#i
int Interpreter::ldrsbPtrp(uint32_t opcode) {
    return ldrsbPt(opcode, rp(opcode));
} // LDRSB Rd,[Rn],Rm
int Interpreter::ldrsbPtip(uint32_t opcode) {
    return ldrsbPt(opcode, ipH(opcode));
} // LDRSB Rd,[Rn],#i
int Interpreter::ldrshOfrm(uint32_t opcode) {
    return ldrshOf(opcode, -rp(opcode));
} // LDRSH Rd,[Rn,-Rm]
int Interpreter::ldrshOfim(uint32_t opcode) {
    return ldrshOf(opcode, -ipH(opcode));
} // LDRSH Rd,[Rn,-#i]
int Interpreter::ldrshOfrp(uint32_t opcode) {
    return ldrshOf(opcode, rp(opcode));
} // LDRSH Rd,[Rn,Rm]
int Interpreter::ldrshOfip(uint32_t opcode) {
    return ldrshOf(opcode, ipH(opcode));
} // LDRSH Rd,[Rn,#i]
int Interpreter::ldrshPrrm(uint32_t opcode) {
    return ldrshPr(opcode, -rp(opcode));
} // LDRSH Rd,[Rn,-Rm]!
int Interpreter::ldrshPrim(uint32_t opcode) {
    return ldrshPr(opcode, -ipH(opcode));
} // LDRSH Rd,[Rn,-#i]!
int Interpreter::ldrshPrrp(uint32_t opcode) {
    return ldrshPr(opcode, rp(opcode));
} // LDRSH Rd,[Rn,Rm]!
int Interpreter::ldrshPrip(uint32_t opcode) {
    return ldrshPr(opcode, ipH(opcode));
} // LDRSH Rd,[Rn,#i]!
int Interpreter::ldrshPtrm(uint32_t opcode) {
    return ldrshPt(opcode, -rp(opcode));
} // LDRSH Rd,[Rn],-Rm
int Interpreter::ldrshPtim(uint32_t opcode) {
    return ldrshPt(opcode, -ipH(opcode));
} // LDRSH Rd,[Rn],-#i
int Interpreter::ldrshPtrp(uint32_t opcode) {
    return ldrshPt(opcode, rp(opcode));
} // LDRSH Rd,[Rn],Rm
int Interpreter::ldrshPtip(uint32_t opcode) {
    return ldrshPt(opcode, ipH(opcode));
} // LDRSH Rd,[Rn],#i

int Interpreter::ldrbOfim(uint32_t opcode) {
    return ldrbOf(opcode, -ip(opcode));
} // LDRB Rd,[Rn,-#i]
int Interpreter::ldrbOfip(uint32_t opcode) { return ldrbOf(opcode, ip(opcode)); } // LDRB Rd,[Rn,#i]
int Interpreter::ldrbOfrmll(uint32_t opcode) {
    return ldrbOf(opcode, -rpll(opcode));
} // LDRB Rd,[Rn,-Rm,LSL #i]
int Interpreter::ldrbOfrmlr(uint32_t opcode) {
    return ldrbOf(opcode, -rplr(opcode));
} // LDRB Rd,[Rn,-Rm,LSR #i]
int Interpreter::ldrbOfrmar(uint32_t opcode) {
    return ldrbOf(opcode, -rpar(opcode));
} // LDRB Rd,[Rn,-Rm,ASR #i]
int Interpreter::ldrbOfrmrr(uint32_t opcode) {
    return ldrbOf(opcode, -rprr(opcode));
} // LDRB Rd,[Rn,-Rm,ROR #i]
int Interpreter::ldrbOfrpll(uint32_t opcode) {
    return ldrbOf(opcode, rpll(opcode));
} // LDRB Rd,[Rn,Rm,LSL #i]
int Interpreter::ldrbOfrplr(uint32_t opcode) {
    return ldrbOf(opcode, rplr(opcode));
} // LDRB Rd,[Rn,Rm,LSR #i]
int Interpreter::ldrbOfrpar(uint32_t opcode) {
    return ldrbOf(opcode, rpar(opcode));
} // LDRB Rd,[Rn,Rm,ASR #i]
int Interpreter::ldrbOfrprr(uint32_t opcode) {
    return ldrbOf(opcode, rprr(opcode));
} // LDRB Rd,[Rn,Rm,ROR #i]
int Interpreter::ldrbPrim(uint32_t opcode) {
    return ldrbPr(opcode, -ip(opcode));
} // LDRB Rd,[Rn,-#i]!
int Interpreter::ldrbPrip(uint32_t opcode) {
    return ldrbPr(opcode, ip(opcode));
} // LDRB Rd,[Rn,#i]!
int Interpreter::ldrbPrrmll(uint32_t opcode) {
    return ldrbPr(opcode, -rpll(opcode));
} // LDRB Rd,[Rn,-Rm,LSL #i]!
int Interpreter::ldrbPrrmlr(uint32_t opcode) {
    return ldrbPr(opcode, -rplr(opcode));
} // LDRB Rd,[Rn,-Rm,LSR #i]!
int Interpreter::ldrbPrrmar(uint32_t opcode) {
    return ldrbPr(opcode, -rpar(opcode));
} // LDRB Rd,[Rn,-Rm,ASR #i]!
int Interpreter::ldrbPrrmrr(uint32_t opcode) {
    return ldrbPr(opcode, -rprr(opcode));
} // LDRB Rd,[Rn,-Rm,ROR #i]!
int Interpreter::ldrbPrrpll(uint32_t opcode) {
    return ldrbPr(opcode, rpll(opcode));
} // LDRB Rd,[Rn,Rm,LSL #i]!
int Interpreter::ldrbPrrplr(uint32_t opcode) {
    return ldrbPr(opcode, rplr(opcode));
} // LDRB Rd,[Rn,Rm,LSR #i]!
int Interpreter::ldrbPrrpar(uint32_t opcode) {
    return ldrbPr(opcode, rpar(opcode));
} // LDRB Rd,[Rn,Rm,ASR #i]!
int Interpreter::ldrbPrrprr(uint32_t opcode) {
    return ldrbPr(opcode, rprr(opcode));
} // LDRB Rd,[Rn,Rm,ROR #i]!
int Interpreter::ldrbPtim(uint32_t opcode) {
    return ldrbPt(opcode, -ip(opcode));
} // LDRB Rd,[Rn],-#i
int Interpreter::ldrbPtip(uint32_t opcode) { return ldrbPt(opcode, ip(opcode)); } // LDRB Rd,[Rn],#i
int Interpreter::ldrbPtrmll(uint32_t opcode) {
    return ldrbPt(opcode, -rpll(opcode));
} // LDRB Rd,[Rn],-Rm,LSL #i
int Interpreter::ldrbPtrmlr(uint32_t opcode) {
    return ldrbPt(opcode, -rplr(opcode));
} // LDRB Rd,[Rn],-Rm,LSR #i
int Interpreter::ldrbPtrmar(uint32_t opcode) {
    return ldrbPt(opcode, -rpar(opcode));
} // LDRB Rd,[Rn],-Rm,ASR #i
int Interpreter::ldrbPtrmrr(uint32_t opcode) {
    return ldrbPt(opcode, -rprr(opcode));
} // LDRB Rd,[Rn],-Rm,ROR #i
int Interpreter::ldrbPtrpll(uint32_t opcode) {
    return ldrbPt(opcode, rpll(opcode));
} // LDRB Rd,[Rn],Rm,LSL #i
int Interpreter::ldrbPtrplr(uint32_t opcode) {
    return ldrbPt(opcode, rplr(opcode));
} // LDRB Rd,[Rn],Rm,LSR #i
int Interpreter::ldrbPtrpar(uint32_t opcode) {
    return ldrbPt(opcode, rpar(opcode));
} // LDRB Rd,[Rn],Rm,ASR #i
int Interpreter::ldrbPtrprr(uint32_t opcode) {
    return ldrbPt(opcode, rprr(opcode));
} // LDRB Rd,[Rn],Rm,ROR #i

int Interpreter::strbOfim(uint32_t opcode) {
    return strbOf(opcode, -ip(opcode));
} // STRB Rd,[Rn,-#i]
int Interpreter::strbOfip(uint32_t opcode) { return strbOf(opcode, ip(opcode)); } // STRB Rd,[Rn,#i]
int Interpreter::strbOfrmll(uint32_t opcode) {
    return strbOf(opcode, -rpll(opcode));
} // STRB Rd,[Rn,-Rm,LSL #i]
int Interpreter::strbOfrmlr(uint32_t opcode) {
    return strbOf(opcode, -rplr(opcode));
} // STRB Rd,[Rn,-Rm,LSR #i]
int Interpreter::strbOfrmar(uint32_t opcode) {
    return strbOf(opcode, -rpar(opcode));
} // STRB Rd,[Rn,-Rm,ASR #i]
int Interpreter::strbOfrmrr(uint32_t opcode) {
    return strbOf(opcode, -rprr(opcode));
} // STRB Rd,[Rn,-Rm,ROR #i]
int Interpreter::strbOfrpll(uint32_t opcode) {
    return strbOf(opcode, rpll(opcode));
} // STRB Rd,[Rn,Rm,LSL #i]
int Interpreter::strbOfrplr(uint32_t opcode) {
    return strbOf(opcode, rplr(opcode));
} // STRB Rd,[Rn,Rm,LSR #i]
int Interpreter::strbOfrpar(uint32_t opcode) {
    return strbOf(opcode, rpar(opcode));
} // STRB Rd,[Rn,Rm,ASR #i]
int Interpreter::strbOfrprr(uint32_t opcode) {
    return strbOf(opcode, rprr(opcode));
} // STRB Rd,[Rn,Rm,ROR #i]
int Interpreter::strbPrim(uint32_t opcode) {
    return strbPr(opcode, -ip(opcode));
} // STRB Rd,[Rn,-#i]!
int Interpreter::strbPrip(uint32_t opcode) {
    return strbPr(opcode, ip(opcode));
} // STRB Rd,[Rn,#i]!
int Interpreter::strbPrrmll(uint32_t opcode) {
    return strbPr(opcode, -rpll(opcode));
} // STRB Rd,[Rn,-Rm,LSL #i]!
int Interpreter::strbPrrmlr(uint32_t opcode) {
    return strbPr(opcode, -rplr(opcode));
} // STRB Rd,[Rn,-Rm,LSR #i]!
int Interpreter::strbPrrmar(uint32_t opcode) {
    return strbPr(opcode, -rpar(opcode));
} // STRB Rd,[Rn,-Rm,ASR #i]!
int Interpreter::strbPrrmrr(uint32_t opcode) {
    return strbPr(opcode, -rprr(opcode));
} // STRB Rd,[Rn,-Rm,ROR #i]!
int Interpreter::strbPrrpll(uint32_t opcode) {
    return strbPr(opcode, rpll(opcode));
} // STRB Rd,[Rn,Rm,LSL #i]!
int Interpreter::strbPrrplr(uint32_t opcode) {
    return strbPr(opcode, rplr(opcode));
} // STRB Rd,[Rn,Rm,LSR #i]!
int Interpreter::strbPrrpar(uint32_t opcode) {
    return strbPr(opcode, rpar(opcode));
} // STRB Rd,[Rn,Rm,ASR #i]!
int Interpreter::strbPrrprr(uint32_t opcode) {
    return strbPr(opcode, rprr(opcode));
} // STRB Rd,[Rn,Rm,ROR #i]!
int Interpreter::strbPtim(uint32_t opcode) {
    return strbPt(opcode, -ip(opcode));
} // STRB Rd,[Rn],-#i
int Interpreter::strbPtip(uint32_t opcode) { return strbPt(opcode, ip(opcode)); } // STRB Rd,[Rn],#i
int Interpreter::strbPtrmll(uint32_t opcode) {
    return strbPt(opcode, -rpll(opcode));
} // STRB Rd,[Rn],-Rm,LSL #i
int Interpreter::strbPtrmlr(uint32_t opcode) {
    return strbPt(opcode, -rplr(opcode));
} // STRB Rd,[Rn],-Rm,LSR #i
int Interpreter::strbPtrmar(uint32_t opcode) {
    return strbPt(opcode, -rpar(opcode));
} // STRB Rd,[Rn],-Rm,ASR #i
int Interpreter::strbPtrmrr(uint32_t opcode) {
    return strbPt(opcode, -rprr(opcode));
} // STRB Rd,[Rn],-Rm,ROR #i
int Interpreter::strbPtrpll(uint32_t opcode) {
    return strbPt(opcode, rpll(opcode));
} // STRB Rd,[Rn],Rm,LSL #i
int Interpreter::strbPtrplr(uint32_t opcode) {
    return strbPt(opcode, rplr(opcode));
} // STRB Rd,[Rn],Rm,LSR #i
int Interpreter::strbPtrpar(uint32_t opcode) {
    return strbPt(opcode, rpar(opcode));
} // STRB Rd,[Rn],Rm,ASR #i
int Interpreter::strbPtrprr(uint32_t opcode) {
    return strbPt(opcode, rprr(opcode));
} // STRB Rd,[Rn],Rm,ROR #i

int Interpreter::ldrhOfrm(uint32_t opcode) {
    return ldrhOf(opcode, -rp(opcode));
} // LDRH Rd,[Rn,-Rm]
int Interpreter::ldrhOfim(uint32_t opcode) {
    return ldrhOf(opcode, -ipH(opcode));
} // LDRH Rd,[Rn,-#i]
int Interpreter::ldrhOfrp(uint32_t opcode) { return ldrhOf(opcode, rp(opcode)); } // LDRH Rd,[Rn,Rm]
int Interpreter::ldrhOfip(uint32_t opcode) {
    return ldrhOf(opcode, ipH(opcode));
} // LDRH Rd,[Rn,#i]
int Interpreter::ldrhPrrm(uint32_t opcode) {
    return ldrhPr(opcode, -rp(opcode));
} // LDRH Rd,[Rn,-Rm]!
int Interpreter::ldrhPrim(uint32_t opcode) {
    return ldrhPr(opcode, -ipH(opcode));
} // LDRH Rd,[Rn,-#i]!
int Interpreter::ldrhPrrp(uint32_t opcode) {
    return ldrhPr(opcode, rp(opcode));
} // LDRH Rd,[Rn,Rm]!
int Interpreter::ldrhPrip(uint32_t opcode) {
    return ldrhPr(opcode, ipH(opcode));
} // LDRH Rd,[Rn,#i]!
int Interpreter::ldrhPtrm(uint32_t opcode) {
    return ldrhPt(opcode, -rp(opcode));
} // LDRH Rd,[Rn],-Rm
int Interpreter::ldrhPtim(uint32_t opcode) {
    return ldrhPt(opcode, -ipH(opcode));
} // LDRH Rd,[Rn],-#i
int Interpreter::ldrhPtrp(uint32_t opcode) { return ldrhPt(opcode, rp(opcode)); } // LDRH Rd,[Rn],Rm
int Interpreter::ldrhPtip(uint32_t opcode) {
    return ldrhPt(opcode, ipH(opcode));
} // LDRH Rd,[Rn],#i
int Interpreter::strhOfrm(uint32_t opcode) {
    return strhOf(opcode, -rp(opcode));
} // STRH Rd,[Rn,-Rm]
int Interpreter::strhOfim(uint32_t opcode) {
    return strhOf(opcode, -ipH(opcode));
} // STRH Rd,[Rn,-#i]
int Interpreter::strhOfrp(uint32_t opcode) { return strhOf(opcode, rp(opcode)); } // STRH Rd,[Rn,Rm]
int Interpreter::strhOfip(uint32_t opcode) {
    return strhOf(opcode, ipH(opcode));
} // STRH Rd,[Rn,#i]
int Interpreter::strhPrrm(uint32_t opcode) {
    return strhPr(opcode, -rp(opcode));
} // STRH Rd,[Rn,-Rm]!
int Interpreter::strhPrim(uint32_t opcode) {
    return strhPr(opcode, -ipH(opcode));
} // STRH Rd,[Rn,-#i]!
int Interpreter::strhPrrp(uint32_t opcode) {
    return strhPr(opcode, rp(opcode));
} // STRH Rd,[Rn,Rm]!
int Interpreter::strhPrip(uint32_t opcode) {
    return strhPr(opcode, ipH(opcode));
} // STRH Rd,[Rn,#i]!
int Interpreter::strhPtrm(uint32_t opcode) {
    return strhPt(opcode, -rp(opcode));
} // STRH Rd,[Rn],-Rm
int Interpreter::strhPtim(uint32_t opcode) {
    return strhPt(opcode, -ipH(opcode));
} // STRH Rd,[Rn],-#i
int Interpreter::strhPtrp(uint32_t opcode) { return strhPt(opcode, rp(opcode)); } // STRH Rd,[Rn],Rm
int Interpreter::strhPtip(uint32_t opcode) {
    return strhPt(opcode, ipH(opcode));
} // STRH Rd,[Rn],#i
...
int Interpreter::ldrOfrmll(uint32_t opcode) {
    return ldrOf(opcode, -rpll(opcode));
} // LDR Rd,[Rn,-Rm,LSL #i]
int Interpreter::ldrOfrmlr(uint32_t opcode) {
    return ldrOf(opcode, -rplr(opcode));
} // LDR Rd,[Rn,-Rm,LSR #i]
int Interpreter::ldrOfrmar(uint32_t opcode) {
    return ldrOf(opcode, -rpar(opcode));
} // LDR Rd,[Rn,-Rm,ASR #i]
int Interpreter::ldrOfrmrr(uint32_t opcode) {
    return ldrOf(opcode, -rprr(opcode));
} // LDR Rd,[Rn,-Rm,ROR #i]
int Interpreter::ldrOfrpll(uint32_t opcode) {
    return ldrOf(opcode, rpll(opcode));
} // LDR Rd,[Rn,Rm,LSL #i]
int Interpreter::ldrOfrplr(uint32_t opcode) {
    return ldrOf(opcode, rplr(opcode));
} // LDR Rd,[Rn,Rm,LSR #i]
int Interpreter::ldrOfrpar(uint32_t opcode) {
    return ldrOf(opcode, rpar(opcode));
} // LDR Rd,[Rn,Rm,ASR #i]
int Interpreter::ldrOfrprr(uint32_t opcode) {
    return ldrOf(opcode, rprr(opcode));
} // LDR Rd,[Rn,Rm,ROR #i]
...
int Interpreter::ldrPrrmll(uint32_t opcode) {
    return ldrPr(opcode, -rpll(opcode));
} // LDR Rd,[Rn,-Rm,LSL #i]!
int Interpreter::ldrPrrmlr(uint32_t opcode) {
    return ldrPr(opcode, -rplr(opcode));
} // LDR Rd,[Rn,-Rm,LSR #i]!
int Interpreter::ldrPrrmar(uint32_t opcode) {
    return ldrPr(opcode, -rpar(opcode));
} // LDR Rd,[Rn,-Rm,ASR #i]!
int Interpreter::ldrPrrmrr(uint32_t opcode) {
    return ldrPr(opcode, -rprr(opcode));
} // LDR Rd,[Rn,-Rm,ROR #i]!
int Interpreter::ldrPrrpll(uint32_t opcode) {
    return ldrPr(opcode, rpll(opcode));
} // LDR Rd,[Rn,Rm,LSL #i]!
int Interpreter::ldrPrrplr(uint32_t opcode) {
    return ldrPr(opcode, rplr(opcode));
} // LDR Rd,[Rn,Rm,LSR #i]!
int Interpreter::ldrPrrpar(uint32_t opcode) {
    return ldrPr(opcode, rpar(opcode));
} // LDR Rd,[Rn,Rm,ASR #i]!
int Interpreter::ldrPrrprr(uint32_t opcode) {
    return ldrPr(opcode, rprr(opcode));
} // LDR Rd,[Rn,Rm,ROR #i]!
...
int Interpreter::ldrPtrmll(uint32_t opcode) {
    return ldrPt(opcode, -rpll(opcode));
} // LDR Rd,[Rn],-Rm,LSL #i
int Interpreter::ldrPtrmlr(uint32_t opcode) {
    return ldrPt(opcode, -rplr(opcode));
} // LDR Rd,[Rn],-Rm,LSR #i
int Interpreter::ldrPtrmar(uint32_t opcode) {
    return ldrPt(opcode, -rpar(opcode));
} // LDR Rd,[Rn],-Rm,ASR #i
int Interpreter::ldrPtrmrr(uint32_t opcode) {
    return ldrPt(opcode, -rprr(opcode));
} // LDR Rd,[Rn],-Rm,ROR #i
int Interpreter::ldrPtrpll(uint32_t opcode) {
    return ldrPt(opcode, rpll(opcode));
} // LDR Rd,[Rn],Rm,LSL #i
int Interpreter::ldrPtrplr(uint32_t opcode) {
    return ldrPt(opcode, rplr(opcode));
} // LDR Rd,[Rn],Rm,LSR #i
int Interpreter::ldrPtrpar(uint32_t opcode) {
    return ldrPt(opcode, rpar(opcode));
} // LDR Rd,[Rn],Rm,ASR #i
int Interpreter::ldrPtrprr(uint32_t opcode) {
    return ldrPt(opcode, rprr(opcode));
} // LDR Rd,[Rn],Rm,ROR #i
...
int Interpreter::strOfrmll(uint32_t opcode) {
    return strOf(opcode, -rpll(opcode));
} // STR Rd,[Rn,-Rm,LSL #i]
int Interpreter::strOfrmlr(uint32_t opcode) {
    return strOf(opcode, -rplr(opcode));
} // STR Rd,[Rn,-Rm,LSR #i]
int Interpreter::strOfrmar(uint32_t opcode) {
    return strOf(opcode, -rpar(opcode));
} // STR Rd,[Rn,-Rm,ASR #i]
int Interpreter::strOfrmrr(uint32_t opcode) {
    return strOf(opcode, -rprr(opcode));
} // STR Rd,[Rn,-Rm,ROR #i]
int Interpreter::strOfrpll(uint32_t opcode) {
    return strOf(opcode, rpll(opcode));
} // STR Rd,[Rn,Rm,LSL #i]
int Interpreter::strOfrplr(uint32_t opcode) {
    return strOf(opcode, rplr(opcode));
} // STR Rd,[Rn,Rm,LSR #i]
int Interpreter::strOfrpar(uint32_t opcode) {
    return strOf(opcode, rpar(opcode));
} // STR Rd,[Rn,Rm,ASR #i]
int Interpreter::strOfrprr(uint32_t opcode) {
    return strOf(opcode, rprr(opcode));
} // STR Rd,[Rn,Rm,ROR #i]
...
int Interpreter::strPrrmll(uint32_t opcode) {
    return strPr(opcode, -rpll(opcode));
} // STR Rd,[Rn,-Rm,LSL #i]!
int Interpreter::strPrrmlr(uint32_t opcode) {
    return strPr(opcode, -rplr(opcode));
} // STR Rd,[Rn,-Rm,LSR #i]!
int Interpreter::strPrrmar(uint32_t opcode) {
    return strPr(opcode, -rpar(opcode));
} // STR Rd,[Rn,-Rm,ASR #i]!
int Interpreter::strPrrmrr(uint32_t opcode) {
    return strPr(opcode, -rprr(opcode));
} // STR Rd,[Rn,-Rm,ROR #i]!
int Interpreter::strPrrpll(uint32_t opcode) {
    return strPr(opcode, rpll(opcode));
} // STR Rd,[Rn,Rm,LSL #i]!
int Interpreter::strPrrplr(uint32_t opcode) {
    return strPr(opcode, rplr(opcode));
} // STR Rd,[Rn,Rm,LSR #i]!
int Interpreter::strPrrpar(uint32_t opcode) {
    return strPr(opcode, rpar(opcode));
} // STR Rd,[Rn,Rm,ASR #i]!
int Interpreter::strPrrprr(uint32_t opcode) {
    return strPr(opcode, rprr(opcode));
} // STR Rd,[Rn,Rm,ROR #i]!
...
int Interpreter::strPtrmll(uint32_t opcode) {
    return strPt(opcode, -rpll(opcode));
} // STR Rd,[Rn],-Rm,LSL #i
int Interpreter::strPtrmlr(uint32_t opcode) {
    return strPt(opcode, -rplr(opcode));
} // STR Rd,[Rn],-Rm,LSR #i
int Interpreter::strPtrmar(uint32_t opcode) {
    return strPt(opcode, -rpar(opcode));
} // STR Rd,[Rn],-Rm,ASR #i
int Interpreter::strPtrmrr(uint32_t opcode) {
    return strPt(opcode, -rprr(opcode));
} // STR Rd,[Rn],-Rm,ROR #i
int Interpreter::strPtrpll(uint32_t opcode) {
    return strPt(opcode, rpll(opcode));
} // STR Rd,[Rn],Rm,LSL #i
int Interpreter::strPtrplr(uint32_t opcode) {
    return strPt(opcode, rplr(opcode));
} // STR Rd,[Rn],Rm,LSR #i
int Interpreter::strPtrpar(uint32_t opcode) {
    return strPt(opcode, rpar(opcode));
} // STR Rd,[Rn],Rm,ASR #i
int Interpreter::strPtrprr(uint32_t opcode) {
    return strPt(opcode, rprr(opcode));
} // STR Rd,[Rn],Rm,ROR #i

int Interpreter::ldrdOfrm(uint32_t opcode) {
    return ldrdOf(opcode, -rp(opcode));
} // LDRD Rd,[Rn,-Rm]
int Interpreter::ldrdOfim(uint32_t opcode) {
    return ldrdOf(opcode, -ipH(opcode));
} // LDRD Rd,[Rn,-#i]
int Interpreter::ldrdOfrp(uint32_t opcode) { return ldrdOf(opcode, rp(opcode)); } // LDRD Rd,[Rn,Rm]
int Interpreter::ldrdOfip(uint32_t opcode) {
    return ldrdOf(opcode, ipH(opcode));
} // LDRD Rd,[Rn,#i]
int Interpreter::ldrdPrrm(uint32_t opcode) {
    return ldrdPr(opcode, -rp(opcode));
} // LDRD Rd,[Rn,-Rm]!
int Interpreter::ldrdPrim(uint32_t opcode) {
    return ldrdPr(opcode, -ipH(opcode));
} // LDRD Rd,[Rn,-#i]!
int Interpreter::ldrdPrrp(uint32_t opcode) {
    return ldrdPr(opcode, rp(opcode));
} // LDRD Rd,[Rn,Rm]!
int Interpreter::ldrdPrip(uint32_t opcode) {
    return ldrdPr(opcode, ipH(opcode));
} // LDRD Rd,[Rn,#i]!
int Interpreter::ldrdPtrm(uint32_t opcode) {
    return ldrdPt(opcode, -rp(opcode));
} // LDRD Rd,[Rn],-Rm
int Interpreter::ldrdPtim(uint32_t opcode) {
    return ldrdPt(opcode, -ipH(opcode));
} // LDRD Rd,[Rn],-#i
int Interpreter::ldrdPtrp(uint32_t opcode) { return ldrdPt(opcode, rp(opcode)); } // LDRD Rd,[Rn],Rm
int Interpreter::ldrdPtip(uint32_t opcode) {
    return ldrdPt(opcode, ipH(opcode));
} // LDRD Rd,[Rn],#i
int Interpreter::strdOfrm(uint32_t opcode) {
    return strdOf(opcode, -rp(opcode));
} // STRD Rd,[Rn,-Rm]
int Interpreter::strdOfim(uint32_t opcode) {
    return strdOf(opcode, -ipH(opcode));
} // STRD Rd,[Rn,-#i]
int Interpreter::strdOfrp(uint32_t opcode) { return strdOf(opcode, rp(opcode)); } // STRD Rd,[Rn,Rm]
int Interpreter::strdOfip(uint32_t opcode) {
    return strdOf(opcode, ipH(opcode));
} // STRD Rd,[Rn,#i]
int Interpreter::strdPrrm(uint32_t opcode) {
    return strdPr(opcode, -rp(opcode));
} // STRD Rd,[Rn,-Rm]!
int Interpreter::strdPrim(uint32_t opcode) {
    return strdPr(opcode, -ipH(opcode));
} // STRD Rd,[Rn,-#i]!
int Interpreter::strdPrrp(uint32_t opcode) {
    return strdPr(opcode, rp(opcode));
} // STRD Rd,[Rn,Rm]!
int Interpreter::strdPrip(uint32_t opcode) {
    return strdPr(opcode, ipH(opcode));
} // STRD Rd,[Rn,#i]!
int Interpreter::strdPtrm(uint32_t opcode) {
    return strdPt(opcode, -rp(opcode));
} // STRD Rd,[Rn],-Rm
int Interpreter::strdPtim(uint32_t opcode) {
    return strdPt(opcode, -ipH(opcode));
} // STRD Rd,[Rn],-#i
int Interpreter::strdPtrp(uint32_t opcode) { return strdPt(opcode, rp(opcode)); } // STRD Rd,[Rn],Rm
int Interpreter::strdPtip(uint32_t opcode) {
    return strdPt(opcode, ipH(opcode));
} // STRD Rd,[Rn],#i
...
int Interpreter::ldmdaW(uint32_t opcode) // LDMDA Rn!, <Rlist>
{
    ...
    if (!(opcode & BIT(m)) || (cpu == 0 && ((opcode & 0x0000FFFF) == BIT(m) ||
                                            (opcode & 0x0000FFFF & ~(BIT(m + 1) - 1)))))
