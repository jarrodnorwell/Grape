...
void Cartridge::trimRom() {
    ...
    if (newSize < romSize) {
        ...
        romSize = newSize;
        auto *newRom = new uint8_t[newSize];
...
void Cartridge::resizeSave(int newSize, bool dirty) {
    mutex.lock();
    auto *newSave = new uint8_t[newSize];
    ...
    for (uint32_t i = 0; i < size9; i += 4) {
        if (romEncrypted && offset9 + i >= 0x4000 && offset9 + i < 0x4800) {
            if (offset9 + i < 0x4008) {
                ...
                core->memory.write<uint32_t>(0, ramAddr9 + i,
                                             data >> (((offset + i) & 4) ? 32 : 0));
            }
        } else {
    ...
    for (uint32_t i = 0; i < size7; i += 4) {
        if (romEncrypted && offset7 + i >= 0x4000 && offset7 + i < 0x4800) {
            if (offset7 + i < 0x4008) {
                ...
                initKeycode(3);
                uint64_t data = decrypt64(U8TO64(rom, (offset + i) & ~7));
                core->memory.write<uint32_t>(1, ramAddr7 + i,
                                             data >> (((offset + i) & 4) ? 32 : 0));
            }
        } else {
...
void CartridgeNds::writeRomCmdOutH(bool cpu, uint32_t mask, uint32_t value) {
    ...
    romCmdOut[cpu] =
            (romCmdOut[cpu] & ~((uint64_t) mask << 32)) | ((uint64_t) (value & mask) << 32);
}
...
void CartridgeNds::writeAuxSpiData(bool cpu, uint8_t value) {
    ...
    if (auxWriteCount[cpu] == 0) {
        ...
        switch (saveSize) {
            ...
            case 0x8000:
            case 0x10000:
            case 0x20000: // EEPROM 8KB, 64KB, 128KB; FRAM 32KB
            {
                switch (auxCommand[cpu]) {
                    case 0x03: // Read from memory
                    {
                        if (auxWriteCount[cpu] < ((saveSize == 0x20000) ? 4 : 3)) {
                            ...
                            auxAddress[cpu] |= value
                                    << ((((saveSize == 0x20000) ? 3 : 2) - auxWriteCount[cpu]) * 8);
                            auxSpiData[cpu] = 0;
                        } else {
                            // On writes 4+, read data from the save and send it back
                            auxSpiData[cpu] = (auxAddress[cpu] < saveSize) ? save[auxAddress[cpu]]
                                                                           : 0;
                            auxAddress[cpu]++;
                        }
                        break;
                    ...
                    case 0x05:
                    {
                        LOG("Write to AUX SPI with EEPROM/FRAM command: 0x%X\n", auxCommand[cpu]);
                        break;
                    }
            ...
            case 0x80000:
            case 0x100000:
            case 0x800000: // FLASH 256KB, 512KB, 1024KB, 8192KB
            {
                switch (auxCommand[cpu]) {
...
void CartridgeNds::writeRomCtrl(bool cpu, uint32_t mask, uint32_t value) {
    ...
    if (rom) {
        ...
        } else if (command == 0x9000000000000000 || (command >> 60) == 0x1 ||
                   command == 0xB800000000000000) // Get chip ID
        {
            cmdMode = CMD_CHIP;
...
uint8_t CartridgeGba::eepromRead() {
    ...
    if (((eepromCmd & 0xC000) >> 14) == 0x3 && eepromCount >= length + 1) // Read
    {
        if (++eepromCount >= length + 6) {
            ...
            uint16_t addr = (saveSize == 0x200) ? ((eepromCmd & 0x3F00) >> 8) : (eepromCmd &
                                                                                 0x03FF);
