...
template<typename T>
T VramMapping::read(uint32_t address) {
    T value = 0;
...
template<typename T>
void VramMapping::write(uint32_t address, T value) {
...
void Memory::updateMap9(uint32_t start, uint32_t end) {
    ...
    for (uint64_t address = start; address < end; address += 0x1000) {
        ...
        if (core->cp15.getItcmReadEnabled() &&
            address < core->cp15.getItcmSize()) // Instruction TCM
        {
...
template<typename T>
T Memory::readFallback(bool cpu, uint32_t address) {
    uint8_t *data = nullptr;
    ...
    } else if (core->isGbaMode()) // GBA
    {
        switch (address & 0xFF000000) {
            case 0x00000000: // GBA BIOS (only readable when executing; otherwise returns last value)
                if (address < 0x4000)
                    data = (core->interpreter[1].getPC() < 0x4000)
                           ? (lastGbaBios = &gbaBios[address]) : lastGbaBios;
                break;
...
template<typename T>
void Memory::writeFallback(bool cpu, uint32_t address, T value) {
    uint8_t *data = nullptr;
...
template<typename T>
T Memory::ioRead9(uint32_t address) {
    T value = 0;
    size_t i = 0;
    ...
    while (i < sizeof(T)) {
        ...
        switch (base) {
            ...
            DEF_IO16(0x400006C,
                     data = core->gpu2D[0].readMasterBright())    // MASTER_BRIGHT (engine A)
            ...
            DEF_IO16(0x400106C,
                     data = core->gpu2D[1].readMasterBright())    // MASTER_BRIGHT (engine B)
...
template<typename T>
T Memory::ioRead7(uint32_t address) {
...
template<typename T>
T Memory::ioReadGba(uint32_t address) {
    T value = 0;
    size_t i = 0;
...
template<typename T>
void Memory::ioWrite9(uint32_t address, T value) {
    size_t i = 0;
    ...
    while (i < sizeof(T)) {
        ...
        switch (base) {
            DEF_IO32(0x4000000,
                     core->gpu2D[0].writeDispCnt(IOWR_PARAMS))            // DISPCNT (engine A)
            DEF_IO16(0x4000004,
                     core->gpu.writeDispStat(0, IOWR_PARAMS))             // DISPSTAT (ARM9)
            DEF_IO16(0x4000008,
                     core->gpu2D[0].writeBgCnt(0, IOWR_PARAMS))           // BG0CNT (engine A)
            DEF_IO16(0x400000A,
                     core->gpu2D[0].writeBgCnt(1, IOWR_PARAMS))           // BG1CNT (engine A)
            DEF_IO16(0x400000C,
                     core->gpu2D[0].writeBgCnt(2, IOWR_PARAMS))           // BG2CNT (engine A)
            DEF_IO16(0x400000E,
                     core->gpu2D[0].writeBgCnt(3, IOWR_PARAMS))           // BG3CNT (engine A)
            DEF_IO16(0x4000010,
                     core->gpu2D[0].writeBgHOfs(0, IOWR_PARAMS))          // BG0HOFS (engine A)
            DEF_IO16(0x4000012,
                     core->gpu2D[0].writeBgVOfs(0, IOWR_PARAMS))          // BG0VOFS (engine A)
            DEF_IO16(0x4000014,
                     core->gpu2D[0].writeBgHOfs(1, IOWR_PARAMS))          // BG1HOFS (engine A)
            DEF_IO16(0x4000016,
                     core->gpu2D[0].writeBgVOfs(1, IOWR_PARAMS))          // BG1VOFS (engine A)
            DEF_IO16(0x4000018,
                     core->gpu2D[0].writeBgHOfs(2, IOWR_PARAMS))          // BG2HOFS (engine A)
            DEF_IO16(0x400001A,
                     core->gpu2D[0].writeBgVOfs(2, IOWR_PARAMS))          // BG2VOFS (engine A)
            DEF_IO16(0x400001C,
                     core->gpu2D[0].writeBgHOfs(3, IOWR_PARAMS))          // BG3HOFS (engine A)
            DEF_IO16(0x400001E,
                     core->gpu2D[0].writeBgVOfs(3, IOWR_PARAMS))          // BG3VOFS (engine A)
            DEF_IO16(0x4000020,
                     core->gpu2D[0].writeBgPA(2, IOWR_PARAMS))            // BG2PA (engine A)
            DEF_IO16(0x4000022,
                     core->gpu2D[0].writeBgPB(2, IOWR_PARAMS))            // BG2PB (engine A)
            DEF_IO16(0x4000024,
                     core->gpu2D[0].writeBgPC(2, IOWR_PARAMS))            // BG2PC (engine A)
            DEF_IO16(0x4000026,
                     core->gpu2D[0].writeBgPD(2, IOWR_PARAMS))            // BG2PD (engine A)
            DEF_IO32(0x4000028,
                     core->gpu2D[0].writeBgX(2, IOWR_PARAMS))             // BG2X (engine A)
            DEF_IO32(0x400002C,
                     core->gpu2D[0].writeBgY(2, IOWR_PARAMS))             // BG2Y (engine A)
            DEF_IO16(0x4000030,
                     core->gpu2D[0].writeBgPA(3, IOWR_PARAMS))            // BG3PA (engine A)
            DEF_IO16(0x4000032,
                     core->gpu2D[0].writeBgPB(3, IOWR_PARAMS))            // BG3PB (engine A)
            DEF_IO16(0x4000034,
                     core->gpu2D[0].writeBgPC(3, IOWR_PARAMS))            // BG3PC (engine A)
            DEF_IO16(0x4000036,
                     core->gpu2D[0].writeBgPD(3, IOWR_PARAMS))            // BG3PD (engine A)
            DEF_IO32(0x4000038,
                     core->gpu2D[0].writeBgX(3, IOWR_PARAMS))             // BG3X (engine A)
            DEF_IO32(0x400003C,
                     core->gpu2D[0].writeBgY(3, IOWR_PARAMS))             // BG3Y (engine A)
            DEF_IO16(0x4000040,
                     core->gpu2D[0].writeWinH(0, IOWR_PARAMS))            // WIN0H (engine A)
            DEF_IO16(0x4000042,
                     core->gpu2D[0].writeWinH(1, IOWR_PARAMS))            // WIN1H (engine A)
            DEF_IO16(0x4000044,
                     core->gpu2D[0].writeWinV(0, IOWR_PARAMS))            // WIN0V (engine A)
            DEF_IO16(0x4000046,
                     core->gpu2D[0].writeWinV(1, IOWR_PARAMS))            // WIN1V (engine A)
            DEF_IO16(0x4000048,
                     core->gpu2D[0].writeWinIn(IOWR_PARAMS))              // WININ (engine A)
            DEF_IO16(0x400004A,
                     core->gpu2D[0].writeWinOut(IOWR_PARAMS))             // WINOUT (engine A)
            DEF_IO16(0x400004C,
                     core->gpu2D[0].writeMosaic(IOWR_PARAMS))             // MOSAIC (engine A)
            DEF_IO16(0x4000050,
                     core->gpu2D[0].writeBldCnt(IOWR_PARAMS))             // BLDCNT (engine A)
            DEF_IO16(0x4000052,
                     core->gpu2D[0].writeBldAlpha(IOWR_PARAMS))           // BLDALPHA (engine A)
            DEF_IO_8(0x4000054,
                     core->gpu2D[0].writeBldY(IOWR_PARAMS8))              // BLDY (engine A)
            ...
            DEF_IO16(0x400006C, core->gpu2D[0].writeMasterBright(
                    IOWR_PARAMS))       // MASTER_BRIGHT (engine A)
            DEF_IO32(0x40000B0,
                     core->dma[0].writeDmaSad(0, IOWR_PARAMS))            // DMA0SAD (ARM9)
            DEF_IO32(0x40000B4,
                     core->dma[0].writeDmaDad(0, IOWR_PARAMS))            // DMA0DAD (ARM9)
            DEF_IO32(0x40000B8,
                     core->dma[0].writeDmaCnt(0, IOWR_PARAMS))            // DMA0CNT (ARM9)
            DEF_IO32(0x40000BC,
                     core->dma[0].writeDmaSad(1, IOWR_PARAMS))            // DMA1SAD (ARM9)
            DEF_IO32(0x40000C0,
                     core->dma[0].writeDmaDad(1, IOWR_PARAMS))            // DMA1DAD (ARM9)
            DEF_IO32(0x40000C4,
                     core->dma[0].writeDmaCnt(1, IOWR_PARAMS))            // DMA1CNT (ARM9)
            DEF_IO32(0x40000C8,
                     core->dma[0].writeDmaSad(2, IOWR_PARAMS))            // DMA2SAD (ARM9)
            DEF_IO32(0x40000CC,
                     core->dma[0].writeDmaDad(2, IOWR_PARAMS))            // DMA2DAD (ARM9)
            DEF_IO32(0x40000D0,
                     core->dma[0].writeDmaCnt(2, IOWR_PARAMS))            // DMA2CNT (ARM9)
            DEF_IO32(0x40000D4,
                     core->dma[0].writeDmaSad(3, IOWR_PARAMS))            // DMA3SAD (ARM9)
            DEF_IO32(0x40000D8,
                     core->dma[0].writeDmaDad(3, IOWR_PARAMS))            // DMA3DAD (ARM9)
            DEF_IO32(0x40000DC,
                     core->dma[0].writeDmaCnt(3, IOWR_PARAMS))            // DMA3CNT (ARM9)
            ...
            DEF_IO16(0x4000100,
                     core->timers[0].writeTmCntL(0, IOWR_PARAMS))         // TM0CNT_L (ARM9)
            DEF_IO16(0x4000102,
                     core->timers[0].writeTmCntH(0, IOWR_PARAMS))         // TM0CNT_H (ARM9)
            DEF_IO16(0x4000104,
                     core->timers[0].writeTmCntL(1, IOWR_PARAMS))         // TM1CNT_L (ARM9)
            DEF_IO16(0x4000106,
                     core->timers[0].writeTmCntH(1, IOWR_PARAMS))         // TM1CNT_H (ARM9)
            DEF_IO16(0x4000108,
                     core->timers[0].writeTmCntL(2, IOWR_PARAMS))         // TM2CNT_L (ARM9)
            DEF_IO16(0x400010A,
                     core->timers[0].writeTmCntH(2, IOWR_PARAMS))         // TM2CNT_H (ARM9)
            DEF_IO16(0x400010C,
                     core->timers[0].writeTmCntL(3, IOWR_PARAMS))         // TM3CNT_L (ARM9)
            DEF_IO16(0x400010E,
                     core->timers[0].writeTmCntH(3, IOWR_PARAMS))         // TM3CNT_H (ARM9)
            DEF_IO16(0x4000180,
                     core->ipc.writeIpcSync(0, IOWR_PARAMS))              // IPCSYNC (ARM9)
            DEF_IO16(0x4000184,
                     core->ipc.writeIpcFifoCnt(0, IOWR_PARAMS))           // IPCFIFOCNT (ARM9)
            DEF_IO32(0x4000188,
                     core->ipc.writeIpcFifoSend(0, IOWR_PARAMS))          // IPCFIFOSEND (ARM9)
            DEF_IO16(0x40001A0,
                     core->cartridgeNds.writeAuxSpiCnt(0, IOWR_PARAMS))   // AUXSPICNT (ARM9)
            DEF_IO_8(0x40001A2,
                     core->cartridgeNds.writeAuxSpiData(0, IOWR_PARAMS8)) // AUXSPIDATA (ARM9)
            DEF_IO32(0x40001A4,
                     core->cartridgeNds.writeRomCtrl(0, IOWR_PARAMS))     // ROMCTRL (ARM9)
            DEF_IO32(0x40001A8,
                     core->cartridgeNds.writeRomCmdOutL(0, IOWR_PARAMS))  // ROMCMDOUT_L (ARM9)
            DEF_IO32(0x40001AC,
                     core->cartridgeNds.writeRomCmdOutH(0, IOWR_PARAMS))  // ROMCMDOUT_H (ARM9)
            ...
            DEF_IO_8(0x4000300,
                     core->interpreter[0].writePostFlg(IOWR_PARAMS8))     // POSTFLG (ARM9)
            ...
            DEF_IO32(0x40004A8,
                     core->gpu3D.writeTexImageParam(IOWR_PARAMS))         // TEXIMAGE_PARAM
            ...
            DEF_IO32(0x4001000,
                     core->gpu2D[1].writeDispCnt(IOWR_PARAMS))            // DISPCNT (engine B)
            DEF_IO16(0x4001008,
                     core->gpu2D[1].writeBgCnt(0, IOWR_PARAMS))           // BG0CNT (engine B)
            DEF_IO16(0x400100A,
                     core->gpu2D[1].writeBgCnt(1, IOWR_PARAMS))           // BG1CNT (engine B)
            DEF_IO16(0x400100C,
                     core->gpu2D[1].writeBgCnt(2, IOWR_PARAMS))           // BG2CNT (engine B)
            DEF_IO16(0x400100E,
                     core->gpu2D[1].writeBgCnt(3, IOWR_PARAMS))           // BG3CNT (engine B)
            DEF_IO16(0x4001010,
                     core->gpu2D[1].writeBgHOfs(0, IOWR_PARAMS))          // BG0HOFS (engine B)
            DEF_IO16(0x4001012,
                     core->gpu2D[1].writeBgVOfs(0, IOWR_PARAMS))          // BG0VOFS (engine B)
            DEF_IO16(0x4001014,
                     core->gpu2D[1].writeBgHOfs(1, IOWR_PARAMS))          // BG1HOFS (engine B)
            DEF_IO16(0x4001016,
                     core->gpu2D[1].writeBgVOfs(1, IOWR_PARAMS))          // BG1VOFS (engine B)
            DEF_IO16(0x4001018,
                     core->gpu2D[1].writeBgHOfs(2, IOWR_PARAMS))          // BG2HOFS (engine B)
            DEF_IO16(0x400101A,
                     core->gpu2D[1].writeBgVOfs(2, IOWR_PARAMS))          // BG2VOFS (engine B)
            DEF_IO16(0x400101C,
                     core->gpu2D[1].writeBgHOfs(3, IOWR_PARAMS))          // BG3HOFS (engine B)
            DEF_IO16(0x400101E,
                     core->gpu2D[1].writeBgVOfs(3, IOWR_PARAMS))          // BG3VOFS (engine B)
            DEF_IO16(0x4001020,
                     core->gpu2D[1].writeBgPA(2, IOWR_PARAMS))            // BG2PA (engine B)
            DEF_IO16(0x4001022,
                     core->gpu2D[1].writeBgPB(2, IOWR_PARAMS))            // BG2PB (engine B)
            DEF_IO16(0x4001024,
                     core->gpu2D[1].writeBgPC(2, IOWR_PARAMS))            // BG2PC (engine B)
            DEF_IO16(0x4001026,
                     core->gpu2D[1].writeBgPD(2, IOWR_PARAMS))            // BG2PD (engine B)
            DEF_IO32(0x4001028,
                     core->gpu2D[1].writeBgX(2, IOWR_PARAMS))             // BG2X (engine B)
            DEF_IO32(0x400102C,
                     core->gpu2D[1].writeBgY(2, IOWR_PARAMS))             // BG2Y (engine B)
            DEF_IO16(0x4001030,
                     core->gpu2D[1].writeBgPA(3, IOWR_PARAMS))            // BG3PA (engine B)
            DEF_IO16(0x4001032,
                     core->gpu2D[1].writeBgPB(3, IOWR_PARAMS))            // BG3PB (engine B)
            DEF_IO16(0x4001034,
                     core->gpu2D[1].writeBgPC(3, IOWR_PARAMS))            // BG3PC (engine B)
            DEF_IO16(0x4001036,
                     core->gpu2D[1].writeBgPD(3, IOWR_PARAMS))            // BG3PD (engine B)
            DEF_IO32(0x4001038,
                     core->gpu2D[1].writeBgX(3, IOWR_PARAMS))             // BG3X (engine B)
            DEF_IO32(0x400103C,
                     core->gpu2D[1].writeBgY(3, IOWR_PARAMS))             // BG3Y (engine B)
            DEF_IO16(0x4001040,
                     core->gpu2D[1].writeWinH(0, IOWR_PARAMS))            // WIN0H (engine B)
            DEF_IO16(0x4001042,
                     core->gpu2D[1].writeWinH(1, IOWR_PARAMS))            // WIN1H (engine B)
            DEF_IO16(0x4001044,
                     core->gpu2D[1].writeWinV(0, IOWR_PARAMS))            // WIN0V (engine B)
            DEF_IO16(0x4001046,
                     core->gpu2D[1].writeWinV(1, IOWR_PARAMS))            // WIN1V (engine B)
            DEF_IO16(0x4001048,
                     core->gpu2D[1].writeWinIn(IOWR_PARAMS))              // WININ (engine B)
            DEF_IO16(0x400104A,
                     core->gpu2D[1].writeWinOut(IOWR_PARAMS))             // WINOUT (engine B)
            DEF_IO16(0x400104C,
                     core->gpu2D[1].writeMosaic(IOWR_PARAMS))             // MOSAIC (engine B)
            DEF_IO16(0x4001050,
                     core->gpu2D[1].writeBldCnt(IOWR_PARAMS))             // BLDCNT (engine B)
            DEF_IO16(0x4001052,
                     core->gpu2D[1].writeBldAlpha(IOWR_PARAMS))           // BLDALPHA (engine B)
            DEF_IO_8(0x4001054,
                     core->gpu2D[1].writeBldY(IOWR_PARAMS8))              // BLDY (engine B)
            DEF_IO16(0x400106C, core->gpu2D[1].writeMasterBright(
                    IOWR_PARAMS))       // MASTER_BRIGHT (engine B)
...
template<typename T>
void Memory::ioWrite7(uint32_t address, T value) {
    // Mirror the WiFi regions
    if (address >= 0x04808000 && address < 0x04810000)
        address &= ~0x00008000;
    ...
    while (i < sizeof(T)) {
        ...
        switch (base) {
            DEF_IO16(0x4000004,
                     core->gpu.writeDispStat(1, IOWR_PARAMS))             // DISPSTAT (ARM7)
            DEF_IO32(0x40000B0,
                     core->dma[1].writeDmaSad(0, IOWR_PARAMS))            // DMA0SAD (ARM7)
            DEF_IO32(0x40000B4,
                     core->dma[1].writeDmaDad(0, IOWR_PARAMS))            // DMA0DAD (ARM7)
            DEF_IO32(0x40000B8,
                     core->dma[1].writeDmaCnt(0, IOWR_PARAMS))            // DMA0CNT (ARM7)
            DEF_IO32(0x40000BC,
                     core->dma[1].writeDmaSad(1, IOWR_PARAMS))            // DMA1SAD (ARM7)
            DEF_IO32(0x40000C0,
                     core->dma[1].writeDmaDad(1, IOWR_PARAMS))            // DMA1DAD (ARM7)
            DEF_IO32(0x40000C4,
                     core->dma[1].writeDmaCnt(1, IOWR_PARAMS))            // DMA1CNT (ARM7)
            DEF_IO32(0x40000C8,
                     core->dma[1].writeDmaSad(2, IOWR_PARAMS))            // DMA2SAD (ARM7)
            DEF_IO32(0x40000CC,
                     core->dma[1].writeDmaDad(2, IOWR_PARAMS))            // DMA2DAD (ARM7)
            DEF_IO32(0x40000D0,
                     core->dma[1].writeDmaCnt(2, IOWR_PARAMS))            // DMA2CNT (ARM7)
            DEF_IO32(0x40000D4,
                     core->dma[1].writeDmaSad(3, IOWR_PARAMS))            // DMA3SAD (ARM7)
            DEF_IO32(0x40000D8,
                     core->dma[1].writeDmaDad(3, IOWR_PARAMS))            // DMA3DAD (ARM7)
            DEF_IO32(0x40000DC,
                     core->dma[1].writeDmaCnt(3, IOWR_PARAMS))            // DMA3CNT (ARM7)
            DEF_IO16(0x4000100,
                     core->timers[1].writeTmCntL(0, IOWR_PARAMS))         // TM0CNT_L (ARM7)
            DEF_IO16(0x4000102,
                     core->timers[1].writeTmCntH(0, IOWR_PARAMS))         // TM0CNT_H (ARM7)
            DEF_IO16(0x4000104,
                     core->timers[1].writeTmCntL(1, IOWR_PARAMS))         // TM1CNT_L (ARM7)
            DEF_IO16(0x4000106,
                     core->timers[1].writeTmCntH(1, IOWR_PARAMS))         // TM1CNT_H (ARM7)
            DEF_IO16(0x4000108,
                     core->timers[1].writeTmCntL(2, IOWR_PARAMS))         // TM2CNT_L (ARM7)
            DEF_IO16(0x400010A,
                     core->timers[1].writeTmCntH(2, IOWR_PARAMS))         // TM2CNT_H (ARM7)
            DEF_IO16(0x400010C,
                     core->timers[1].writeTmCntL(3, IOWR_PARAMS))         // TM3CNT_L (ARM7)
            DEF_IO16(0x400010E,
                     core->timers[1].writeTmCntH(3, IOWR_PARAMS))         // TM3CNT_H (ARM7)

            DEF_IO16(0x4000134, rCnt = IOWR_PARAMS)

            DEF_IO_8(0x4000138, core->rtc.writeRtc(IOWR_PARAMS8))                    // RTC
            DEF_IO16(0x4000180,
                     core->ipc.writeIpcSync(1, IOWR_PARAMS))              // IPCSYNC (ARM7)
            DEF_IO16(0x4000184,
                     core->ipc.writeIpcFifoCnt(1, IOWR_PARAMS))           // IPCFIFOCNT (ARM7)
            DEF_IO32(0x4000188,
                     core->ipc.writeIpcFifoSend(1, IOWR_PARAMS))          // IPCFIFOSEND (ARM7)
            DEF_IO16(0x40001A0,
                     core->cartridgeNds.writeAuxSpiCnt(1, IOWR_PARAMS))   // AUXSPICNT (ARM7)
            DEF_IO_8(0x40001A2,
                     core->cartridgeNds.writeAuxSpiData(1, IOWR_PARAMS8)) // AUXSPIDATA (ARM7)
            DEF_IO32(0x40001A4,
                     core->cartridgeNds.writeRomCtrl(1, IOWR_PARAMS))     // ROMCTRL (ARM7)
            DEF_IO32(0x40001A8,
                     core->cartridgeNds.writeRomCmdOutL(1, IOWR_PARAMS))  // ROMCMDOUT_L (ARM7)
            DEF_IO32(0x40001AC,
                     core->cartridgeNds.writeRomCmdOutH(1, IOWR_PARAMS))  // ROMCMDOUT_H (ARM7)
            ...
            DEF_IO_8(0x4000300,
                     core->interpreter[1].writePostFlg(IOWR_PARAMS8))     // POSTFLG (ARM7)
            ...
            DEF_IO16(0x4800050,
                     core->wifi.writeWRxbufBegin(IOWR_PARAMS))            // W_RXBUF_BEGIN
            DEF_IO16(0x4800052, core->wifi.writeWRxbufEnd(IOWR_PARAMS))              // W_RXBUF_END
            DEF_IO16(0x4800056,
                     core->wifi.writeWRxbufWrAddr(IOWR_PARAMS))           // W_RXBUF_WR_ADDR
            DEF_IO16(0x4800058,
                     core->wifi.writeWRxbufRdAddr(IOWR_PARAMS))           // W_RXBUF_RD_ADDR
            DEF_IO16(0x480005A,
                     core->wifi.writeWRxbufReadcsr(IOWR_PARAMS))          // W_RXBUF_READCSR
            DEF_IO16(0x480005C,
                     core->wifi.writeWRxbufCount(IOWR_PARAMS))            // W_RXBUF_COUNT
            DEF_IO16(0x4800062, core->wifi.writeWRxbufGap(IOWR_PARAMS))              // W_RXBUF_GAP
            DEF_IO16(0x4800064,
                     core->wifi.writeWRxbufGapdisp(IOWR_PARAMS))          // W_RXBUF_GAPDISP
            DEF_IO16(0x4800068,
                     core->wifi.writeWTxbufWrAddr(IOWR_PARAMS))           // W_TXBUF_WR_ADDR
            DEF_IO16(0x480006C,
                     core->wifi.writeWTxbufCount(IOWR_PARAMS))            // W_TXBUF_COUNT
            DEF_IO16(0x4800070,
                     core->wifi.writeWTxbufWrData(IOWR_PARAMS))           // W_TXBUF_WR_DATA
            DEF_IO16(0x4800074, core->wifi.writeWTxbufGap(IOWR_PARAMS))              // W_TXBUF_GAP
            DEF_IO16(0x4800076,
                     core->wifi.writeWTxbufGapdisp(IOWR_PARAMS))          // W_TXBUF_GAPDISP
            DEF_IO16(0x4800080,
                     core->wifi.writeWTxbufLoc(4, IOWR_PARAMS))           // W_TXBUF_BEACON
            ...
            DEF_IO16(0x48000AC,
                     core->wifi.writeWTxreqReset(IOWR_PARAMS))            // W_TXREQ_RESET
            DEF_IO16(0x48000AE, core->wifi.writeWTxreqSet(IOWR_PARAMS))              // W_TXREQ_SET
            DEF_IO16(0x48000E8,
                     core->wifi.writeWUsCountcnt(IOWR_PARAMS))            // W_US_COUNTCNT
            DEF_IO16(0x48000EA,
                     core->wifi.writeWUsComparecnt(IOWR_PARAMS))          // W_US_COMPARECNT
            DEF_IO16(0x4800110, core->wifi.writeWPreBeacon(IOWR_PARAMS))             // W_PRE_BEACON
            DEF_IO16(0x480011C,
                     core->wifi.writeWBeaconCount(IOWR_PARAMS))           // W_BEACON_COUNT
            ...
            DEF_IO16(0x4800134,
                     core->wifi.writeWPostBeacon(IOWR_PARAMS))            // W_POST_BEACON
...
template<typename T>
void Memory::ioWriteGba(uint32_t address, T value) {
    size_t i = 0;
