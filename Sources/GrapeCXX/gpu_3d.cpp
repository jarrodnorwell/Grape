...
Matrix Matrix::operator*(Matrix &mtx) {
    ...
    for (int y = 0; y < 4; y++) {
        for (int x = 0; x < 4; x++) {
            result.data[y * 4 + x] = ((int64_t) data[y * 4 + 0] * mtx.data[0 + x] +
                                      (int64_t) data[y * 4 + 1] * mtx.data[4 + x] +
                                      (int64_t) data[y * 4 + 2] * mtx.data[8 + x] +
                                      (int64_t) data[y * 4 + 3] * mtx.data[12 + x]) >> 12;
        }
    }
...
Vector Vector::operator*(Matrix &mtx) {
    ...
    result.x = ((int64_t) x * mtx.data[0] + (int64_t) y * mtx.data[4] + (int64_t) z * mtx.data[8])
            >> 12;
    result.y = ((int64_t) x * mtx.data[1] + (int64_t) y * mtx.data[5] + (int64_t) z * mtx.data[9])
            >> 12;
    result.z = ((int64_t) x * mtx.data[2] + (int64_t) y * mtx.data[6] + (int64_t) z * mtx.data[10])
            >> 12;
...
Vertex Vertex::operator*(Matrix &mtx) {
    ...
    result.x = ((int64_t) x * mtx.data[0] + (int64_t) y * mtx.data[4] + (int64_t) z * mtx.data[8] +
                (int64_t) w * mtx.data[12]) >> 12;
    result.y = ((int64_t) x * mtx.data[1] + (int64_t) y * mtx.data[5] + (int64_t) z * mtx.data[9] +
                (int64_t) w * mtx.data[13]) >> 12;
    result.z = ((int64_t) x * mtx.data[2] + (int64_t) y * mtx.data[6] + (int64_t) z * mtx.data[10] +
                (int64_t) w * mtx.data[14]) >> 12;
    result.w = ((int64_t) x * mtx.data[3] + (int64_t) y * mtx.data[7] + (int64_t) z * mtx.data[11] +
                (int64_t) w * mtx.data[15]) >> 12;
...
Gpu3D::Gpu3D(Core *core) : core(core) {
    ...
    runCommandTask = [this] { runCommand(); };
}
...
Vertex Gpu3D::intersection(Vertex *vtx1, Vertex *vtx2, int32_t val1, int32_t val2) {
    ...
    uint8_t r =
            ((d2 * ((vtx1->color >> 0) & 0x3F)) - (d1 * ((vtx2->color >> 0) & 0x3F))) / (d2 - d1);
    uint8_t g =
            ((d2 * ((vtx1->color >> 6) & 0x3F)) - (d1 * ((vtx2->color >> 6) & 0x3F))) / (d2 - d1);
    uint8_t b =
            ((d2 * ((vtx1->color >> 12) & 0x3F)) - (d1 * ((vtx2->color >> 12) & 0x3F))) / (d2 - d1);
...
void Gpu3D::runCommand() {
    ...
    if (fifo.empty()) gxStat &= ~BIT(27); // Commands not executing
    ...
    switch ((gxStat & 0xC0000000) >> 30) {
        case 1:
            if (gxStat & BIT(25)) core->interpreter[0].sendInterrupt(21);
            break;
        case 2:
            if (gxStat & BIT(26)) core->interpreter[0].sendInterrupt(21);
            break;
    }
...
void Gpu3D::swapBuffers() {
    ...
    for (int i = 0; i < vertexCountIn; i++) {
        if (verticesIn[i].w != 0) {
            verticesIn[i].x =
                    (((int64_t) verticesIn[i].x + verticesIn[i].w) * w / (verticesIn[i].w * 2) +
                     x) & xMask;
            verticesIn[i].y =
                    ((-(int64_t) verticesIn[i].y + verticesIn[i].w) * h / (verticesIn[i].w * 2) +
                     y) & yMask;
            verticesIn[i].z = (((((int64_t) verticesIn[i].z << 14) / verticesIn[i].w) + 0x3FFF)
                    << 9);
        }
    }
...
void Gpu3D::addVertex() {
    ...
    switch (polygonType) {
        case 0:
            if (vertexCount % 3 == 0) addPolygon();
            break; // Separate triangles
        case 1:
            if (vertexCount % 4 == 0) addPolygon();
            break; // Separate quads
        case 2:
            if (vertexCount >= 3) addPolygon();
            break; // Triangle strips
        case 3:
            if (vertexCount >= 4 && vertexCount % 2 == 0) addPolygon();
            break; // Quad strips
    }
}
...
void Gpu3D::addPolygon() {
    ...
    if (polygonType == 3) SWAP(unclipped[2], unclipped[3]);
    ...
    polygonsIn[polygonCountIn].crossed = (polygonType == 3 && !clip1);
...
void Gpu3D::normalCmd(uint32_t param) {
    ...
    for (int i = 0; i < 4; i++) {
        if (enabledLights & BIT(i)) {
            ...
            r += (((specularColor >> 0) & 0x3F) * ((lightColor[i] >> 0) & 0x3F) * shininessLevel)
                    >> 18;
            g += (((specularColor >> 6) & 0x3F) * ((lightColor[i] >> 6) & 0x3F) * shininessLevel)
                    >> 18;
            b += (((specularColor >> 12) & 0x3F) * ((lightColor[i] >> 12) & 0x3F) * shininessLevel)
                    >> 18;

            r += (((diffuseColor >> 0) & 0x3F) * ((lightColor[i] >> 0) & 0x3F) * diffuseLevel)
                    >> 18;
            g += (((diffuseColor >> 6) & 0x3F) * ((lightColor[i] >> 6) & 0x3F) * diffuseLevel)
                    >> 18;
            b += (((diffuseColor >> 12) & 0x3F) * ((lightColor[i] >> 12) & 0x3F) * diffuseLevel)
                    >> 18;
...
void Gpu3D::addEntry(Entry entry) {
    ...
    switch (entry.command) {
        ...
        case 0x70:
        case 0x71:
        case 0x72: // BOX_TEST, POS_TEST, VEC_TEST
...
uint32_t Gpu3D::readRamCount() const {
