    ...
    for (int i = 0; i < 240; i++) {
        ...
        switch (mode) {
            ...
            alpha:
            {
                ...
                uint8_t r = std::min(
                        (((layers[0][i] >> 0) & 0x1F) * eva + ((layers[1][i] >> 0) & 0x1F) * evb) /
                        16, 31U);
                uint8_t g = std::min(
                        (((layers[0][i] >> 5) & 0x1F) * eva + ((layers[1][i] >> 5) & 0x1F) * evb) /
                        16, 31U);
                uint8_t b = std::min((((layers[0][i] >> 10) & 0x1F) * eva +
                                      ((layers[1][i] >> 10) & 0x1F) * evb) / 16, 31U);
...
void Gpu2D::drawScanline(int line) {
    ...
    for (int i = 0; i < 256; i++) {
        ...
        if (layers[0][i] & BIT(26)) // 3D pixel
        {
            if (bldCnt & BIT(8 + blendBits[1][i])) // Below pixel is blendable
            {
                ...
                uint8_t evb = 64 - eva;
                uint8_t r = std::min(
                        (((layers[0][i] >> 0) & 0x3F) * eva + ((blend >> 0) & 0x3F) * evb) / 64,
                        63U);
                uint8_t g = std::min(
                        (((layers[0][i] >> 6) & 0x3F) * eva + ((blend >> 6) & 0x3F) * evb) / 64,
                        63U);
                uint8_t b = std::min(
                        (((layers[0][i] >> 12) & 0x3F) * eva + ((blend >> 12) & 0x3F) * evb) / 64,
                        63U);
        ...
        switch (mode) {
            ...
            alpha:
            {
                ...
                uint8_t evb = std::min((bldAlpha >> 8) & 0x1F, 16);
                uint8_t r = std::min(
                        (((layers[0][i] >> 0) & 0x3F) * eva + ((blend >> 0) & 0x3F) * evb) / 16,
                        63U);
                uint8_t g = std::min(
                        (((layers[0][i] >> 6) & 0x3F) * eva + ((blend >> 6) & 0x3F) * evb) / 16,
                        63U);
                uint8_t b = std::min(
                        (((layers[0][i] >> 12) & 0x3F) * eva + ((blend >> 12) & 0x3F) * evb) / 16,
                        63U);
                layers[0][i] = (b << 12) | (g << 6) | r;
                continue;
            }
    ...
    switch ((dispCnt >> 16) & 0x3) // Display mode
    {
        ...
        case 2: // VRAM display
        {
            ...
            uint32_t address =
                    0x6800000 + ((dispCnt & 0x000C0000) >> 18) * 0x20000 + line * 256 * 2;
            for (int i = 0; i < 256; i++)
                framebuffer[line * 256 + i] = rgb5ToRgb6(
                        core->memory.read<uint16_t>(false, address + i * 2));
            break;
        }
...
void Gpu2D::drawText(int bg, int line) {
    ...
    int yOffset = (((bgCnt[bg] & BIT(6)) ? (line - (line % (((mosaic >> 4) & 0xF) + 1))) : line) +
                   bgVOfs[bg]) & 0x1FF;
    ...
    if (bgCnt[bg] & BIT(7)) // 8-bit
    {
        for (int i = 0; i <= (gbaMode ? 240 : 256); i += 8) {
            ...
            uint32_t indexAddr = indexBase + (tile & 0x3FF) * 64 +
                                 ((tile & BIT(11)) ? (7 - (yOffset & 7)) : (yOffset & 7)) * 8;
            uint64_t indices = core->memory.read<uint32_t>(gbaMode, indexAddr) |
                               ((uint64_t) core->memory.read<uint32_t>(gbaMode, indexAddr + 4)
                                       << 32);

            // Draw the current line of the tile, flipped horizontally if enabled
            for (int x = i - (xOffset & 7) + ((tile & BIT(10)) ? 7 : 0);
                 indices != 0; ((tile & BIT(10)) ? x-- : x++), indices >>= 8) {
    ...
    } else // 4-bit
    {
        for (int i = 0; i <= (gbaMode ? 240 : 256); i += 8) {
            ...
            // Get the palette indices for the current line of the tile, flipped vertically if enabled
            uint32_t indexAddr = indexBase + (tile & 0x3FF) * 32 +
                                 ((tile & BIT(11)) ? (7 - (yOffset & 7)) : (yOffset & 7)) * 4;
            uint32_t indices = core->memory.read<uint32_t>(gbaMode, indexAddr);

            // Draw the current line of the tile, flipped horizontally if enabled
            for (int x = i - (xOffset & 7) + ((tile & BIT(10)) ? 7 : 0);
                 indices != 0; ((tile & BIT(10)) ? x-- : x++), indices >>= 4) {
                // Draw a pixel if it's visible
...
void Gpu2D::drawExtended(int bg, int line) {
    ...
    } else // Extended affine
    {
        ...
        uint32_t tileBase = bgVramAddr + ((bgCnt[bg] << 3) & 0x0F800) + ((dispCnt >> 11) & 0x70000);
        uint32_t indexBase =
                bgVramAddr + ((bgCnt[bg] << 12) & 0x3C000) + ((dispCnt >> 8) & 0x70000);
        ...
        for (int i = 0; i < 256; i++) {
            ...
            uint32_t indexAddr = indexBase + (tile & 0x3FF) * 64 + // Tile offset
                                 (((tile & BIT(11)) ? (7 - y) : y) & 7) * 8 +
                                 // Vertical offset, flipped if enabled
                                 (((tile & BIT(10)) ? (7 - x) : x) &
                                  7);            // Horizontal offset, flipped if enabled
...
void Gpu2D::drawObjects(int line, bool window) {
    ...
    for (int i = 0; i < 128; i++) {
        ...
        int spriteY =
                ((object[0] & BIT(12)) ? (line - (line % (((mosaic >> 12) & 0xF) + 1))) : line) - y;
        ...
        if (!gbaMode && type == 3) {
            ...
            } else // 2D mapping
            {
                ...
                dataBase = objVramAddr + (object[2] & xMask) * 0x10 +
            ...
            if (object[0] & BIT(8)) // Rotscale
            {
                ...
                for (int j = 0; j < width2; j++) {
                    ...
                    int rotscaleX =
                            ((params[0] * (j - width2 / 2) + params[1] * (spriteY - height2 / 2))
                                    >> 8) + width / 2;
                    if (rotscaleX < 0 || rotscaleX >= width) continue;

                    // Calculate the rotscaled Y coordinate relative to the object
                    int rotscaleY =
                            ((params[2] * (j - width2 / 2) + params[3] * (spriteY - height2 / 2))
                                    >> 8) + height / 2;
                    if (rotscaleY < 0 || rotscaleY >= height) continue;

                    // Draw a pixel
                    uint16_t pixel = core->memory.read<uint16_t>(0, dataBase +
                                                                    (rotscaleY * bitmapWidth +
                                                                     rotscaleX) * 2);
                    if (pixel & BIT(15))
                ...
                dataBase +=
                        ((object[1] & BIT(13)) ? (height - spriteY - 1) : spriteY) * bitmapWidth *
                        2;
            ...
            if (object[0] & BIT(13)) // 8-bit
            {
                ...
                for (int j = 0; j < width2; j++) {
                    ...
                    if (index && type == 2) // Object window
                    {
                        ...
                        if (index)
                            drawObjPixel(line, offset,
                                         ((type == 1) << 25) | BIT(15) | U8TO16(pal, index * 2),
                                         priority);
                        ...
                        if (gbaMode && priority < priorities[0][offset] &&
                            blendBits[0][offset] == 4)
...
void Gpu2D::writeWinH(int win, uint16_t mask, uint16_t value) {
    ...
    if ((winHFlip[win] = (winX1[win] > winX2[win]))) SWAP(winX1[win], winX2[win]);
}
...
void Gpu2D::writeWinV(int win, uint16_t mask, uint16_t value) {
    ...
    if (((winVFlip[win] = (winY1[win] > winY2[win])))) SWAP(winY1[win], winY2[win]);
}
