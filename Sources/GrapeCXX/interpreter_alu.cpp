...
FORCE_INLINE uint32_t Interpreter::llr(uint32_t opcode) // Rm,LSL Rs
{
    ...
    uint32_t value =
            *registers[opcode & 0x0000000F] + (((opcode & 0x0000000F) == 0x0000000F) ? 4 : 0);
...
FORCE_INLINE uint32_t Interpreter::rri(uint32_t opcode) // Rm,ROR #i
{
    ...
    return shift ? ((value << (32 - shift)) | (value >> shift)) : (((cpsr & BIT(29)) << 2) |
                                                                   (value >> 1));
}
...
FORCE_INLINE uint32_t Interpreter::rriS(uint32_t opcode) // Rm,ROR #i (S)
{
    ...
    // Rotate right by immediate
    // A shift of 0 translates to a rotate with carry of 1
    uint32_t res = shift ? ((value << (32 - shift)) | (value >> shift)) : (((cpsr & BIT(29)) << 2) |
                                                                           (value >> 1));
...
FORCE_INLINE int Interpreter::_and(uint32_t opcode, uint32_t op2) // AND Rd,Rn,op2
{
    ...
    uint32_t op1 = *registers[(opcode & 0x000F0000) >> 16] +
                   (((opcode & 0x020F0010) == 0x000F0010) ? 4 : 0);
...
int Interpreter::andLli(uint32_t opcode) {
    return _and(opcode, lli(opcode));
} // AND  Rd,Rn,Rm,LSL #i
int Interpreter::andLlr(uint32_t opcode) {
    return _and(opcode, llr(opcode)) + 1;
} // AND  Rd,Rn,Rm,LSL Rs
int Interpreter::andLri(uint32_t opcode) {
    return _and(opcode, lri(opcode));
} // AND  Rd,Rn,Rm,LSR #i
int Interpreter::andLrr(uint32_t opcode) {
    return _and(opcode, lrr(opcode)) + 1;
} // AND  Rd,Rn,Rm,LSR Rs
int Interpreter::andAri(uint32_t opcode) {
    return _and(opcode, ari(opcode));
} // AND  Rd,Rn,Rm,ASR #i
int Interpreter::andArr(uint32_t opcode) {
    return _and(opcode, arr(opcode)) + 1;
} // AND  Rd,Rn,Rm,ASR Rs
int Interpreter::andRri(uint32_t opcode) {
    return _and(opcode, rri(opcode));
} // AND  Rd,Rn,Rm,ROR #i
int Interpreter::andRrr(uint32_t opcode) {
    return _and(opcode, rrr(opcode)) + 1;
} // AND  Rd,Rn,Rm,ROR Rs
int Interpreter::andImm(uint32_t opcode) { return _and(opcode, imm(opcode)); } // AND  Rd,Rn,#i
int Interpreter::andsLli(uint32_t opcode) {
    return ands(opcode, lliS(opcode));
} // ANDS Rd,Rn,Rm,LSL #i
int Interpreter::andsLlr(uint32_t opcode) {
    return ands(opcode, llrS(opcode)) + 1;
} // ANDS Rd,Rn,Rm,LSL Rs
int Interpreter::andsLri(uint32_t opcode) {
    return ands(opcode, lriS(opcode));
} // ANDS Rd,Rn,Rm,LSR #i
int Interpreter::andsLrr(uint32_t opcode) {
    return ands(opcode, lrrS(opcode)) + 1;
} // ANDS Rd,Rn,Rm,LSR Rs
int Interpreter::andsAri(uint32_t opcode) {
    return ands(opcode, ariS(opcode));
} // ANDS Rd,Rn,Rm,ASR #i
int Interpreter::andsArr(uint32_t opcode) {
    return ands(opcode, arrS(opcode)) + 1;
} // ANDS Rd,Rn,Rm,ASR Rs
int Interpreter::andsRri(uint32_t opcode) {
    return ands(opcode, rriS(opcode));
} // ANDS Rd,Rn,Rm,ROR #i
int Interpreter::andsRrr(uint32_t opcode) {
...
int Interpreter::eorLli(uint32_t opcode) {
    return eor(opcode, lli(opcode));
} // EOR  Rd,Rn,Rm,LSL #i
int Interpreter::eorLlr(uint32_t opcode) {
    return eor(opcode, llr(opcode)) + 1;
} // EOR  Rd,Rn,Rm,LSL Rs
int Interpreter::eorLri(uint32_t opcode) {
    return eor(opcode, lri(opcode));
} // EOR  Rd,Rn,Rm,LSR #i
int Interpreter::eorLrr(uint32_t opcode) {
    return eor(opcode, lrr(opcode)) + 1;
} // EOR  Rd,Rn,Rm,LSR Rs
int Interpreter::eorAri(uint32_t opcode) {
    return eor(opcode, ari(opcode));
} // EOR  Rd,Rn,Rm,ASR #i
int Interpreter::eorArr(uint32_t opcode) {
    return eor(opcode, arr(opcode)) + 1;
} // EOR  Rd,Rn,Rm,ASR Rs
int Interpreter::eorRri(uint32_t opcode) {
    return eor(opcode, rri(opcode));
} // EOR  Rd,Rn,Rm,ROR #i
int Interpreter::eorRrr(uint32_t opcode) {
    return eor(opcode, rrr(opcode)) + 1;
} // EOR  Rd,Rn,Rm,ROR Rs
int Interpreter::eorImm(uint32_t opcode) { return eor(opcode, imm(opcode)); } // EOR  Rd,Rn,#i
int Interpreter::eorsLli(uint32_t opcode) {
    return eors(opcode, lliS(opcode));
} // EORS Rd,Rn,Rm,LSL #i
int Interpreter::eorsLlr(uint32_t opcode) {
    return eors(opcode, llrS(opcode)) + 1;
} // EORS Rd,Rn,Rm,LSL Rs
int Interpreter::eorsLri(uint32_t opcode) {
    return eors(opcode, lriS(opcode));
} // EORS Rd,Rn,Rm,LSR #i
int Interpreter::eorsLrr(uint32_t opcode) {
    return eors(opcode, lrrS(opcode)) + 1;
} // EORS Rd,Rn,Rm,LSR Rs
int Interpreter::eorsAri(uint32_t opcode) {
    return eors(opcode, ariS(opcode));
} // EORS Rd,Rn,Rm,ASR #i
int Interpreter::eorsArr(uint32_t opcode) {
    return eors(opcode, arrS(opcode)) + 1;
} // EORS Rd,Rn,Rm,ASR Rs
int Interpreter::eorsRri(uint32_t opcode) {
    return eors(opcode, rriS(opcode));
} // EORS Rd,Rn,Rm,ROR #i
int Interpreter::eorsRrr(uint32_t opcode) {
...
int Interpreter::subLli(uint32_t opcode) {
    return sub(opcode, lli(opcode));
} // SUB  Rd,Rn,Rm,LSL #i
int Interpreter::subLlr(uint32_t opcode) {
    return sub(opcode, llr(opcode)) + 1;
} // SUB  Rd,Rn,Rm,LSL Rs
int Interpreter::subLri(uint32_t opcode) {
    return sub(opcode, lri(opcode));
} // SUB  Rd,Rn,Rm,LSR #i
int Interpreter::subLrr(uint32_t opcode) {
    return sub(opcode, lrr(opcode)) + 1;
} // SUB  Rd,Rn,Rm,LSR Rs
int Interpreter::subAri(uint32_t opcode) {
    return sub(opcode, ari(opcode));
} // SUB  Rd,Rn,Rm,ASR #i
int Interpreter::subArr(uint32_t opcode) {
    return sub(opcode, arr(opcode)) + 1;
} // SUB  Rd,Rn,Rm,ASR Rs
int Interpreter::subRri(uint32_t opcode) {
    return sub(opcode, rri(opcode));
} // SUB  Rd,Rn,Rm,ROR #i
int Interpreter::subRrr(uint32_t opcode) {
    return sub(opcode, rrr(opcode)) + 1;
} // SUB  Rd,Rn,Rm,ROR Rs
int Interpreter::subImm(uint32_t opcode) { return sub(opcode, imm(opcode)); } // SUB  Rd,Rn,#i
int Interpreter::subsLli(uint32_t opcode) {
    return subs(opcode, lli(opcode));
} // SUBS Rd,Rn,Rm,LSL #i
int Interpreter::subsLlr(uint32_t opcode) {
    return subs(opcode, llr(opcode)) + 1;
} // SUBS Rd,Rn,Rm,LSL Rs
int Interpreter::subsLri(uint32_t opcode) {
    return subs(opcode, lri(opcode));
} // SUBS Rd,Rn,Rm,LSR #i
int Interpreter::subsLrr(uint32_t opcode) {
    return subs(opcode, lrr(opcode)) + 1;
} // SUBS Rd,Rn,Rm,LSR Rs
int Interpreter::subsAri(uint32_t opcode) {
    return subs(opcode, ari(opcode));
} // SUBS Rd,Rn,Rm,ASR #i
int Interpreter::subsArr(uint32_t opcode) {
    return subs(opcode, arr(opcode)) + 1;
} // SUBS Rd,Rn,Rm,ASR Rs
int Interpreter::subsRri(uint32_t opcode) {
    return subs(opcode, rri(opcode));
} // SUBS Rd,Rn,Rm,ROR #i
int Interpreter::subsRrr(uint32_t opcode) {
...
int Interpreter::rsbLli(uint32_t opcode) {
    return rsb(opcode, lli(opcode));
} // RSB  Rd,Rn,Rm,LSL #i
int Interpreter::rsbLlr(uint32_t opcode) {
    return rsb(opcode, llr(opcode)) + 1;
} // RSB  Rd,Rn,Rm,LSL Rs
int Interpreter::rsbLri(uint32_t opcode) {
    return rsb(opcode, lri(opcode));
} // RSB  Rd,Rn,Rm,LSR #i
int Interpreter::rsbLrr(uint32_t opcode) {
    return rsb(opcode, lrr(opcode)) + 1;
} // RSB  Rd,Rn,Rm,LSR Rs
int Interpreter::rsbAri(uint32_t opcode) {
    return rsb(opcode, ari(opcode));
} // RSB  Rd,Rn,Rm,ASR #i
int Interpreter::rsbArr(uint32_t opcode) {
    return rsb(opcode, arr(opcode)) + 1;
} // RSB  Rd,Rn,Rm,ASR Rs
int Interpreter::rsbRri(uint32_t opcode) {
    return rsb(opcode, rri(opcode));
} // RSB  Rd,Rn,Rm,ROR #i
int Interpreter::rsbRrr(uint32_t opcode) {
    return rsb(opcode, rrr(opcode)) + 1;
} // RSB  Rd,Rn,Rm,ROR Rs
int Interpreter::rsbImm(uint32_t opcode) { return rsb(opcode, imm(opcode)); } // RSB  Rd,Rn,#i
int Interpreter::rsbsLli(uint32_t opcode) {
    return rsbs(opcode, lli(opcode));
} // RSBS Rd,Rn,Rm,LSL #i
int Interpreter::rsbsLlr(uint32_t opcode) {
    return rsbs(opcode, llr(opcode)) + 1;
} // RSBS Rd,Rn,Rm,LSL Rs
int Interpreter::rsbsLri(uint32_t opcode) {
    return rsbs(opcode, lri(opcode));
} // RSBS Rd,Rn,Rm,LSR #i
int Interpreter::rsbsLrr(uint32_t opcode) {
    return rsbs(opcode, lrr(opcode)) + 1;
} // RSBS Rd,Rn,Rm,LSR Rs
int Interpreter::rsbsAri(uint32_t opcode) {
    return rsbs(opcode, ari(opcode));
} // RSBS Rd,Rn,Rm,ASR #i
int Interpreter::rsbsArr(uint32_t opcode) {
    return rsbs(opcode, arr(opcode)) + 1;
} // RSBS Rd,Rn,Rm,ASR Rs
int Interpreter::rsbsRri(uint32_t opcode) {
    return rsbs(opcode, rri(opcode));
} // RSBS Rd,Rn,Rm,ROR #i
int Interpreter::rsbsRrr(uint32_t opcode) {
...
int Interpreter::addLli(uint32_t opcode) {
    return add(opcode, lli(opcode));
} // ADD  Rd,Rn,Rm,LSL #i
int Interpreter::addLlr(uint32_t opcode) {
    return add(opcode, llr(opcode)) + 1;
} // ADD  Rd,Rn,Rm,LSL Rs
int Interpreter::addLri(uint32_t opcode) {
    return add(opcode, lri(opcode));
} // ADD  Rd,Rn,Rm,LSR #i
int Interpreter::addLrr(uint32_t opcode) {
    return add(opcode, lrr(opcode)) + 1;
} // ADD  Rd,Rn,Rm,LSR Rs
int Interpreter::addAri(uint32_t opcode) {
    return add(opcode, ari(opcode));
} // ADD  Rd,Rn,Rm,ASR #i
int Interpreter::addArr(uint32_t opcode) {
    return add(opcode, arr(opcode)) + 1;
} // ADD  Rd,Rn,Rm,ASR Rs
int Interpreter::addRri(uint32_t opcode) {
    return add(opcode, rri(opcode));
} // ADD  Rd,Rn,Rm,ROR #i
int Interpreter::addRrr(uint32_t opcode) {
    return add(opcode, rrr(opcode)) + 1;
} // ADD  Rd,Rn,Rm,ROR Rs
int Interpreter::addImm(uint32_t opcode) { return add(opcode, imm(opcode)); } // ADD  Rd,Rn,#i
int Interpreter::addsLli(uint32_t opcode) {
    return adds(opcode, lli(opcode));
} // ADDS Rd,Rn,Rm,LSL #i
int Interpreter::addsLlr(uint32_t opcode) {
    return adds(opcode, llr(opcode)) + 1;
} // ADDS Rd,Rn,Rm,LSL Rs
int Interpreter::addsLri(uint32_t opcode) {
    return adds(opcode, lri(opcode));
} // ADDS Rd,Rn,Rm,LSR #i
int Interpreter::addsLrr(uint32_t opcode) {
    return adds(opcode, lrr(opcode)) + 1;
} // ADDS Rd,Rn,Rm,LSR Rs
int Interpreter::addsAri(uint32_t opcode) {
    return adds(opcode, ari(opcode));
} // ADDS Rd,Rn,Rm,ASR #i
int Interpreter::addsArr(uint32_t opcode) {
    return adds(opcode, arr(opcode)) + 1;
} // ADDS Rd,Rn,Rm,ASR Rs
int Interpreter::addsRri(uint32_t opcode) {
    return adds(opcode, rri(opcode));
} // ADDS Rd,Rn,Rm,ROR #i
int Interpreter::addsRrr(uint32_t opcode) {
...
int Interpreter::adcLli(uint32_t opcode) {
    return adc(opcode, lli(opcode));
} // ADC  Rd,Rn,Rm,LSL #i
int Interpreter::adcLlr(uint32_t opcode) {
    return adc(opcode, llr(opcode)) + 1;
} // ADC  Rd,Rn,Rm,LSL Rs
int Interpreter::adcLri(uint32_t opcode) {
    return adc(opcode, lri(opcode));
} // ADC  Rd,Rn,Rm,LSR #i
int Interpreter::adcLrr(uint32_t opcode) {
    return adc(opcode, lrr(opcode)) + 1;
} // ADC  Rd,Rn,Rm,LSR Rs
int Interpreter::adcAri(uint32_t opcode) {
    return adc(opcode, ari(opcode));
} // ADC  Rd,Rn,Rm,ASR #i
int Interpreter::adcArr(uint32_t opcode) {
    return adc(opcode, arr(opcode)) + 1;
} // ADC  Rd,Rn,Rm,ASR Rs
int Interpreter::adcRri(uint32_t opcode) {
    return adc(opcode, rri(opcode));
} // ADC  Rd,Rn,Rm,ROR #i
int Interpreter::adcRrr(uint32_t opcode) {
    return adc(opcode, rrr(opcode)) + 1;
} // ADC  Rd,Rn,Rm,ROR Rs
int Interpreter::adcImm(uint32_t opcode) { return adc(opcode, imm(opcode)); } // ADC  Rd,Rn,#i
int Interpreter::adcsLli(uint32_t opcode) {
    return adcs(opcode, lli(opcode));
} // ADCS Rd,Rn,Rm,LSL #i
int Interpreter::adcsLlr(uint32_t opcode) {
    return adcs(opcode, llr(opcode)) + 1;
} // ADCS Rd,Rn,Rm,LSL Rs
int Interpreter::adcsLri(uint32_t opcode) {
    return adcs(opcode, lri(opcode));
} // ADCS Rd,Rn,Rm,LSR #i
int Interpreter::adcsLrr(uint32_t opcode) {
    return adcs(opcode, lrr(opcode)) + 1;
} // ADCS Rd,Rn,Rm,LSR Rs
int Interpreter::adcsAri(uint32_t opcode) {
    return adcs(opcode, ari(opcode));
} // ADCS Rd,Rn,Rm,ASR #i
int Interpreter::adcsArr(uint32_t opcode) {
    return adcs(opcode, arr(opcode)) + 1;
} // ADCS Rd,Rn,Rm,ASR Rs
int Interpreter::adcsRri(uint32_t opcode) {
    return adcs(opcode, rri(opcode));
} // ADCS Rd,Rn,Rm,ROR #i
int Interpreter::adcsRrr(uint32_t opcode) {
...
int Interpreter::sbcLli(uint32_t opcode) {
    return sbc(opcode, lli(opcode));
} // SBC  Rd,Rn,Rm,LSL #i
int Interpreter::sbcLlr(uint32_t opcode) {
    return sbc(opcode, llr(opcode)) + 1;
} // SBC  Rd,Rn,Rm,LSL Rs
int Interpreter::sbcLri(uint32_t opcode) {
    return sbc(opcode, lri(opcode));
} // SBC  Rd,Rn,Rm,LSR #i
int Interpreter::sbcLrr(uint32_t opcode) {
    return sbc(opcode, lrr(opcode)) + 1;
} // SBC  Rd,Rn,Rm,LSR Rs
int Interpreter::sbcAri(uint32_t opcode) {
    return sbc(opcode, ari(opcode));
} // SBC  Rd,Rn,Rm,ASR #i
int Interpreter::sbcArr(uint32_t opcode) {
    return sbc(opcode, arr(opcode)) + 1;
} // SBC  Rd,Rn,Rm,ASR Rs
int Interpreter::sbcRri(uint32_t opcode) {
    return sbc(opcode, rri(opcode));
} // SBC  Rd,Rn,Rm,ROR #i
int Interpreter::sbcRrr(uint32_t opcode) {
    return sbc(opcode, rrr(opcode)) + 1;
} // SBC  Rd,Rn,Rm,ROR Rs
int Interpreter::sbcImm(uint32_t opcode) { return sbc(opcode, imm(opcode)); } // SBC  Rd,Rn,#i
int Interpreter::sbcsLli(uint32_t opcode) {
    return sbcs(opcode, lli(opcode));
} // SBCS Rd,Rn,Rm,LSL #i
int Interpreter::sbcsLlr(uint32_t opcode) {
    return sbcs(opcode, llr(opcode)) + 1;
} // SBCS Rd,Rn,Rm,LSL Rs
int Interpreter::sbcsLri(uint32_t opcode) {
    return sbcs(opcode, lri(opcode));
} // SBCS Rd,Rn,Rm,LSR #i
int Interpreter::sbcsLrr(uint32_t opcode) {
    return sbcs(opcode, lrr(opcode)) + 1;
} // SBCS Rd,Rn,Rm,LSR Rs
int Interpreter::sbcsAri(uint32_t opcode) {
    return sbcs(opcode, ari(opcode));
} // SBCS Rd,Rn,Rm,ASR #i
int Interpreter::sbcsArr(uint32_t opcode) {
    return sbcs(opcode, arr(opcode)) + 1;
} // SBCS Rd,Rn,Rm,ASR Rs
int Interpreter::sbcsRri(uint32_t opcode) {
    return sbcs(opcode, rri(opcode));
} // SBCS Rd,Rn,Rm,ROR #i
int Interpreter::sbcsRrr(uint32_t opcode) {
...
int Interpreter::rscLli(uint32_t opcode) {
    return rsc(opcode, lli(opcode));
} // RSC  Rd,Rn,Rm,LSL #i
int Interpreter::rscLlr(uint32_t opcode) {
    return rsc(opcode, llr(opcode)) + 1;
} // RSC  Rd,Rn,Rm,LSL Rs
int Interpreter::rscLri(uint32_t opcode) {
    return rsc(opcode, lri(opcode));
} // RSC  Rd,Rn,Rm,LSR #i
int Interpreter::rscLrr(uint32_t opcode) {
    return rsc(opcode, lrr(opcode)) + 1;
} // RSC  Rd,Rn,Rm,LSR Rs
int Interpreter::rscAri(uint32_t opcode) {
    return rsc(opcode, ari(opcode));
} // RSC  Rd,Rn,Rm,ASR #i
int Interpreter::rscArr(uint32_t opcode) {
    return rsc(opcode, arr(opcode)) + 1;
} // RSC  Rd,Rn,Rm,ASR Rs
int Interpreter::rscRri(uint32_t opcode) {
    return rsc(opcode, rri(opcode));
} // RSC  Rd,Rn,Rm,ROR #i
int Interpreter::rscRrr(uint32_t opcode) {
    return rsc(opcode, rrr(opcode)) + 1;
} // RSC  Rd,Rn,Rm,ROR Rs
int Interpreter::rscImm(uint32_t opcode) { return rsc(opcode, imm(opcode)); } // RSC  Rd,Rn,#i
int Interpreter::rscsLli(uint32_t opcode) {
    return rscs(opcode, lli(opcode));
} // RSCS Rd,Rn,Rm,LSL #i
int Interpreter::rscsLlr(uint32_t opcode) {
    return rscs(opcode, llr(opcode)) + 1;
} // RSCS Rd,Rn,Rm,LSL Rs
int Interpreter::rscsLri(uint32_t opcode) {
    return rscs(opcode, lri(opcode));
} // RSCS Rd,Rn,Rm,LSR #i
int Interpreter::rscsLrr(uint32_t opcode) {
    return rscs(opcode, lrr(opcode)) + 1;
} // RSCS Rd,Rn,Rm,LSR Rs
int Interpreter::rscsAri(uint32_t opcode) {
    return rscs(opcode, ari(opcode));
} // RSCS Rd,Rn,Rm,ASR #i
int Interpreter::rscsArr(uint32_t opcode) {
    return rscs(opcode, arr(opcode)) + 1;
} // RSCS Rd,Rn,Rm,ASR Rs
int Interpreter::rscsRri(uint32_t opcode) {
    return rscs(opcode, rri(opcode));
} // RSCS Rd,Rn,Rm,ROR #i
int Interpreter::rscsRrr(uint32_t opcode) {
