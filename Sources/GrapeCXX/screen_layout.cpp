...
void ScreenLayout::update(int winWidth, int winHeight, bool gbaMode) {
    screenRotation = 0; screenArrangement = 1; screenSizing = 0; screenGap = 0;
    ...
    if (gbaMode && gbaCrop) {
        ...
        float scale = ((baseRatio > screenRatio) ? ((float) winWidth / width) : ((float) winHeight /
                                                                                 height));
        ...
        topX = 0; // topX = (winWidth - topWidth) / 2;
        topY = 0; // // topY = (winHeight - topHeight) / 2;
    } else // NDS mode
    {
        ...
        if (vertical) {
            ...
            if (screenSizing == 0) // Even
            {
                ...
                float screenRatio = (float) winWidth / winHeight;
                largeScale = ((baseRatio > screenRatio) ? ((float) winWidth / width) : (
                        (float) winHeight / (height/* * 2*/)));
            ...
            } else // Enlarge Top/Bottom
            {
                ...
                largeScale = ((baseRatio > largeRatio) ? ((float) winWidth / width) : (
                        (float) (winHeight - height) / height));
                if (integerScale) largeScale = (int) largeScale;

                // Scale the small screen to the remaining window space
                float smallRatio = (float) winWidth / (winHeight - largeScale * height);
                smallScale = ((baseRatio > smallRatio) ? ((float) winWidth / width) : (
                        (float) (winHeight - largeScale * height) / height));
        ...
        } else // Horizontal
        {
            ...
            if (screenSizing == 0) // Even
            {
                ...
                float screenRatio = (float) winWidth / winHeight;
                largeScale = ((baseRatio > screenRatio) ? ((float) winWidth / (width * 2)) : (
            ...
            } else // Enlarge Top/Enlarge Bottom
            {
                ...
                largeScale = ((baseRatio > largeRatio) ? ((float) (winWidth - width) / width) : (
                        (float) winHeight / height));
                if (integerScale) largeScale = (int) largeScale;

                // Scale the small screen to the remaining window space
                float smallRatio = (float) (winWidth - largeScale * width) / winHeight;
                smallScale = ((baseRatio > smallRatio) ? ((float) (winWidth - largeScale * width) /
                                                          width) : ((float) winHeight / height));
                if (integerScale) smallScale = (int) smallScale;
            }
        ...
        if (vertical) {
            ...
            {
                topY = 0; // topY = (winHeight - topHeight - botHeight) / 2;
                botY = 0; // topY + topHeight;
...
int ScreenLayout::getTouchX(int x, int y) {
    ...
    switch (screenRotation) {
        case 0:
            return (x - botX) * 256 / botWidth;  // None
        case 1:
            return (y - botY) * 256 / botHeight; // Clockwise
        default:
            return 255 - (y - botY) * 256 / botHeight; // Counter-clockwise
    }
}
...
int ScreenLayout::getTouchY(int x, int y) {
    ...
    switch (screenRotation) {
        case 0:
            return (y - botY) * 192 / botHeight; // None
        case 1:
            return 191 - (x - botX) * 192 / botWidth;  // Clockwise
        default:
            return (x - botX) * 192 / botWidth;  // Counter-clockwise
    }
}
