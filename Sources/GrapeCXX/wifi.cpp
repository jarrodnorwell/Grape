    ...
    for (size_t i = 0; i < packets.size(); i++) {
        ...
        for (size_t j = 0; j < size; j++) {
            ...
            if ((wRxbufBegin & 0x1FFE) != (wRxbufEnd & 0x1FFE))
                wRxbufWrcsr = (wRxbufBegin & 0x1FFE) + (wRxbufWrcsr - (wRxbufBegin & 0x1FFE)) %
                                                       ((wRxbufEnd & 0x1FFE) -
                                                        (wRxbufBegin & 0x1FFE));
            wRxbufWrcsr &= 0x1FFE;
        }
    ...
    switch ((value & 0xF000) >> 12) {
        case 5: // Write
        {
            if ((index >= 0x01 && index <= 0x0C) || (index >= 0x13 && index <= 0x15) ||
                ...
                (index >= 0x28 && index <= 0x4C) || (index >= 0x4E && index <= 0x5C) ||
...
uint16_t Wifi::readWRxbufRdData() {
    ...
    if ((wRxbufBegin & 0x1FFE) != (wRxbufEnd & 0x1FFE))
        wRxbufRdAddr = (wRxbufBegin & 0x1FFE) + (wRxbufRdAddr - (wRxbufBegin & 0x1FFE)) %
                                                ((wRxbufEnd & 0x1FFE) - (wRxbufBegin & 0x1FFE));
    wRxbufRdAddr &= 0x1FFF;
