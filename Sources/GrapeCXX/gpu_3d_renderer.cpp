...
void Gpu3DRenderer::drawThreaded(int thread) {
    ...
    for (i = thread; i < end; i += activeThreads) {
        ...
        while ((prev > 0 && ready[prev - 1].load() < 2) || ready[prev].load() < 2 ||
               ready[prev + 1].load() < 2)
    ...
    while (ready[prev - 1].load() < 2 || ready[prev].load() < 2 ||
           (prev < 191 && ready[prev + 1].load() < 2))
        std::this_thread::yield();
...
void Gpu3DRenderer::drawScanline1(int line) {
    ...
    uint32_t color =
            BIT(26) | rgba5ToRgba6(((clearColor & 0x001F0000) >> 1) | (clearColor & 0x00007FFF));
...
void Gpu3DRenderer::finishScanline(int line) {
    ...
    if (disp3DCnt & BIT(5)) {
        ...
        for (int i = offset; i <= offset + w; i++) {
            if (attribBuffer[0][i] & BIT(14)) // Edge bit
            {
                ...
                uint32_t id[4] =
                        {
                                (((i & w) > 0) ? attribBuffer[0][i - 1] : (clearColor >> 24)) &
                                0x3F, // Left
                                (((i & w) < w) ? attribBuffer[0][i + 1] : (clearColor >> 24)) &
                                0x3F, // Right
                                ((line > 0) ? attribBuffer[0][i - 512] : (clearColor >> 24)) &
                                0x3F, // Up
                                ((line < h) ? attribBuffer[0][i + 512] : (clearColor >> 24)) &
                                0x3F  // Down
                        };
                ...
                int32_t depth[4] =
                        {
                                (((i & w) > 0) ? depthBuffer[0][i - 1] : ((clearDepth == 0x7FFF)
                                                                          ? 0xFFFFFF : (clearDepth
                                                << 9))), // Left
                                (((i & w) < w) ? depthBuffer[0][i + 1] : ((clearDepth == 0x7FFF)
                                                                          ? 0xFFFFFF : (clearDepth
                                                << 9))), // Right
                                ((line > 0) ? depthBuffer[0][i - 512] : ((clearDepth == 0x7FFF)
                                                                         ? 0xFFFFFF : (clearDepth
                                                << 9))), // Up
                                ((line < h) ? depthBuffer[0][i + 512] : ((clearDepth == 0x7FFF)
                                                                         ? 0xFFFFFF : (clearDepth
                                                << 9)))  // Down
                        };
                ...
                for (int j = 0; j < 4; j++) {
                    if ((attribBuffer[0][i] & 0x3F) != id[j] && depthBuffer[0][i] < depth[j]) {
                        framebuffer[0][i] = BIT(26) | rgba5ToRgba6(
    ...
    if (disp3DCnt & BIT(7)) {
        ...
        for (int layer = 0; layer < ((disp3DCnt & BIT(4)) ? 2
                                                          : 1); layer++) // Apply to the back layer as well if anti-aliased
        {
            int start = line * 256 * 2, end = start + (256 << resShift);
            for (int i = start; i < end; i++) {
                if (attribBuffer[layer][i] & BIT(13)) // Fog bit
                {
                    ...
                    } else // Linear interpolation
                    {
                        ...
                        density = ((m >= 0) ? ((fogTable[n + 1] * m + fogTable[n] * (fogStep - m)) /
                                               fogStep) : fogTable[0]);
                    }
                    ...
                    uint8_t a = (((fog >> 18) & 0x3F) * density +
                                 ((framebuffer[layer][i] >> 18) & 0x3F) * (128 - density)) / 128;
                    if (disp3DCnt & BIT(6)) // Only alpha
                    {
                        framebuffer[layer][i] = (framebuffer[layer][i] & ~(0x3F << 18)) | (a << 18);
                    } else {
                        uint8_t r = (((fog >> 0) & 0x3F) * density +
                                     ((framebuffer[layer][i] >> 0) & 0x3F) * (128 - density)) / 128;
                        uint8_t g = (((fog >> 6) & 0x3F) * density +
                                     ((framebuffer[layer][i] >> 6) & 0x3F) * (128 - density)) / 128;
        ...
        for (int i = start; i < end; i++) {
            if (((attribBuffer[0][i] >> 15) & 0x3F) < 0x3F) // Edge not opaque
            {
                ...
                if ((framebuffer[1][i] >> 18) & 0x3F)
                    framebuffer[0][i] = BIT(26) |
                                        interpolateColor(framebuffer[1][i], framebuffer[0][i], 0,
                                                         ((attribBuffer[0][i] >> 15) & 0x3F), 0x3F);
                else
                    framebuffer[0][i] = (framebuffer[0][i] & ~0xFC0000) |
                                        ((attribBuffer[0][i] & 0x1F8000) << 3);
            }
        }
    }
}
...
uint32_t
Gpu3DRenderer::interpolateLinear(uint32_t v1, uint32_t v2, uint32_t x1, uint32_t x, uint32_t x2) {
    if (x <= x1) return v1;
    if (x >= x2) return v2;
...
uint32_t
Gpu3DRenderer::interpolateLinRev(uint32_t v1, uint32_t v2, uint32_t x1, uint32_t x, uint32_t x2) {
    if (x <= x1) return v1;
    if (x >= x2) return v2;
...
uint32_t
Gpu3DRenderer::interpolateFactor(uint32_t factor, uint32_t shift, uint32_t v1, uint32_t v2) {
...
uint32_t
Gpu3DRenderer::interpolateColor(uint32_t c1, uint32_t c2, uint32_t x1, uint32_t x, uint32_t x2) {
    ...
    uint32_t a = (((c1 >> 18) & 0x3F) > ((c2 >> 18) & 0x3F)) ? ((c1 >> 18) & 0x3F) : ((c2 >> 18) &
                                                                                      0x3F);
    return (a << 18) | (b << 12) | (g << 6) | r;
}
...
void Gpu3DRenderer::drawPolygon(int line, int polygonIndex) {
    ...
    } else if (abs(vertices[v[1]]->x - vertices[v[0]]->x) >
               vertices[v[1]]->y - vertices[v[0]]->y) // X-major
    {
        // Interpolate with an extra bit of precision so the result can be rounded
        x1 = interpolateLinear(vertices[v[0]]->x << 1, vertices[v[1]]->x << 1, vertices[v[0]]->y,
                               line, vertices[v[1]]->y) + 1;
        x2 = interpolateLinear(vertices[v[0]]->x << 1, vertices[v[1]]->x << 1, vertices[v[0]]->y,
                               line + 1, vertices[v[1]]->y) + 1;
    ...
    } else if (vertices[v[2]]->x == vertices[v[3]]->x) // Vertical
    {
        ...
        x3 = vertices[v[2]]->x;
        if ((vertices[v[0]]->x != vertices[v[1]]->x || vertices[v[0]]->x != vertices[v[2]]->x) &&
            x3 > 0)
            x3--;
        x4 = x3;
    } else if (abs(vertices[v[3]]->x - vertices[v[2]]->x) >
               vertices[v[3]]->y - vertices[v[2]]->y) // X-major
    {
        // Interpolate with an extra bit of precision so the result can be rounded
        x3 = interpolateLinear(vertices[v[2]]->x << 1, vertices[v[3]]->x << 1, vertices[v[2]]->y,
                               line, vertices[v[3]]->y) + 1;
        x4 = interpolateLinear(vertices[v[2]]->x << 1, vertices[v[3]]->x << 1, vertices[v[2]]->y,
                               line + 1, vertices[v[3]]->y) + 1;
    ...
    if (abs(vertices[v[1]]->x - vertices[v[0]]->x) >
        abs(vertices[v[1]]->y - vertices[v[0]]->y)) // X-major
    {
    ...
    if (abs(vertices[v[3]]->x - vertices[v[2]]->x) >
        abs(vertices[v[3]]->y - vertices[v[2]]->y)) // X-major
    {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        ze[i] = interpolateLinear(vertices[v[i2]]->z, vertices[v[i2 + 1]]->z, xe1[i], xe[i],
                                  xe2[i]);
        ...
        if (ws[i2] == ws[i2 + 1] && !(ws[i2] & 0x00FE)) {
            ...
            re[i] = interpolateLinear(((vertices[v[i2]]->color >> 0) & 0x3F) << 3,
                                      ((vertices[v[i2 + 1]]->color >> 0) & 0x3F) << 3, xe1[i],
                                      xe[i], xe2[i]);
            ge[i] = interpolateLinear(((vertices[v[i2]]->color >> 6) & 0x3F) << 3,
                                      ((vertices[v[i2 + 1]]->color >> 6) & 0x3F) << 3, xe1[i],
                                      xe[i], xe2[i]);
            be[i] = interpolateLinear(((vertices[v[i2]]->color >> 12) & 0x3F) << 3,
                                      ((vertices[v[i2 + 1]]->color >> 12) & 0x3F) << 3, xe1[i],
                                      xe[i], xe2[i]);
            ...
            se[i] = interpolateLinear((int32_t) vertices[v[i2]]->s + 0xFFFF,
                                      (int32_t) vertices[v[i2 + 1]]->s + 0xFFFF, xe1[i], xe[i],
                                      xe2[i]) - 0xFFFF;
            te[i] = interpolateLinear((int32_t) vertices[v[i2]]->t + 0xFFFF,
                                      (int32_t) vertices[v[i2 + 1]]->t + 0xFFFF, xe1[i], xe[i],
                                      xe2[i]) - 0xFFFF;
        } else {
    ...
    for (uint32_t x = x1; x < x4; x++) {
        ...
        if (polygon->depthTestEqual) {
            ...
            depthPass[0] = (depthBuffer[0][i] >= depth - margin &&
                            depthBuffer[0][i] <= depth + margin);
            depthPass[1] = (disp3DCnt & BIT(4)) && (attribBuffer[0][i] & BIT(14)) &&
                           (depthBuffer[1][i] >= depth - margin &&
                            depthBuffer[1][i] <= depth + margin);
        } else {
        ...
        uint32_t color =
                ((polygon->alpha ? polygon->alpha : 0x3F) << 18) | (bv << 12) | (gv << 6) | rv;
        ...
        if (polygon->textureFmt != 0) {
            ...
            int s, t;
            if (factor == -1) {
                s = (int) (interpolateLinear(se[0] + 0xFFFF, se[1] + 0xFFFF, x1, x, x4) - 0xFFFF)
                        >> 4;
                t = (int) (interpolateLinear(te[0] + 0xFFFF, te[1] + 0xFFFF, x1, x, x4) - 0xFFFF)
                        >> 4;
            } else {
                s = (int) (interpolateFactor(factor, 8, se[0] + 0xFFFF, se[1] + 0xFFFF) - 0xFFFF)
                        >> 4;
                t = (int) (interpolateFactor(factor, 8, te[0] + 0xFFFF, te[1] + 0xFFFF) - 0xFFFF)
                        >> 4;
            }
            ...
            switch (polygon->mode) {
                ...
                {
                    uint8_t r =
                            ((((texel >> 0) & 0x3F) + 1) * (((color >> 0) & 0x3F) + 1) - 1) / 64;
                    uint8_t g =
                            ((((texel >> 6) & 0x3F) + 1) * (((color >> 6) & 0x3F) + 1) - 1) / 64;
                    uint8_t b =
                            ((((texel >> 12) & 0x3F) + 1) * (((color >> 12) & 0x3F) + 1) - 1) / 64;
                    uint8_t a =
                ...
                case 3: // Shadow
                {
                    ...
                    uint8_t r =
                            (((texel >> 0) & 0x3F) * at + ((color >> 0) & 0x3F) * (63 - at)) / 64;
                    uint8_t g =
                            (((texel >> 6) & 0x3F) * at + ((color >> 6) & 0x3F) * (63 - at)) / 64;
                    uint8_t b =
                            (((texel >> 12) & 0x3F) * at + ((color >> 12) & 0x3F) * (63 - at)) / 64;
        ...
        if (!(color & 0xFC0000) ||
            ((x < x1e || x >= x4e) && ((color >> 18) == 0x3F || !(disp3DCnt & BIT(3)))))
            continue;
        ...
        } else if (!(attribBuffer[layer][i] & BIT(12)) ||
                   ((attribBuffer[layer][i] >> 6) & 0x3F) != polygon->id) // Transparent
        {
            // Transparent pixels are only drawn if the old pixel isn't transparent or the polygon ID differs
            framebuffer[layer][i] =
                    BIT(26) | (((disp3DCnt & BIT(3)) && (framebuffer[layer][i] & 0xFC0000)) ?
            ...
            attribBuffer[layer][i] =
                    (attribBuffer[layer][i] & (0x1FC03F | (polygon->fog << 13))) | BIT(12) |
                    (polygon->id << 6);
